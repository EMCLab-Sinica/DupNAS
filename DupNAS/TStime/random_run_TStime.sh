...
mkdir -p "$BASE_OUTDIR"
SELECTED_LIST="$BASE_OUTDIR/${MODEL_NAME}_random${SAMPLE_N}_seed${RANDOM_SEED}_selected.txt"
...
for VM_SIZE in "${VMS[@]}"; do
  ...
  CSV="$OUTDIR/${MODEL_NAME}_vm${VM_SIZE}_dupnasa_run_times.csv"
  SUMMARY="$OUTDIR/${MODEL_NAME}_vm${VM_SIZE}_dupnasa_summary_times.csv"
...
OVERALL_SUMMARY="$SCRIPT_DIR/${MODEL_NAME}_random${SAMPLE_N}_seed${RANDOM_SEED}_300runs_summary.csv"
{
  echo "metric,value"
  echo "model,$MODEL_NAME"
...
echo "================== ALL VM DONE =================="
echo "[Overall TS Runtime among 300 results]"
echo "  model      : $MODEL_NAME"
echo "  ok_count   : $TOTAL_OK"
echo "  fail_count : $TOTAL_FAIL"
