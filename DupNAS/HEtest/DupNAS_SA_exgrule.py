...
parser.add_argument("--mode", type=str, default="dupnas", choices=["dupnas", "tinyts", "patchts", "nots", "count"],
                    help="Choose one of: dupnas, tinyts, patchts, count, nots (nots for tracing memory usage only)")
...
parser.add_argument("--random_seed", type=int, default=None,
                    help="Random seed for sampling model-level TS combinations in count mode")
parser.add_argument("--random_seeds", type=str, default=None,
                    help="Comma-separated or space-separated random seeds for multi-seed sampling in count mode")
parser.add_argument("--random_samples", type=int, default=100,
                    help="Number of random model-level TS combinations to sample per seed in count mode")
parser.add_argument("--exgrule", type=str, default="neither",
                    choices=["neither", "PConly", "BPonly", "both", "boundary", "branches", "one"],
                    help=(
                        "Select which rule-excluded g options are kept in count/random mode. "
                        "New names: neither (= old both), PConly (= old boundary), BPonly (= old branches). "
                        "Old names are accepted for backward compatibility."
                    ))
args = parser.parse_args()

# Normalize old EXG argument names to the new names used by shell scripts.
#   old both      -> neither
#   old boundary  -> PConly
#   old branches  -> BPonly
_EXGRULE_ALIAS = {"both": "neither", "boundary": "PConly", "branches": "BPonly"}
args.exgrule = _EXGRULE_ALIAS.get(args.exgrule, args.exgrule)
# Mode flags
mode = args.mode
...
# === Count candidate combinations for random search ===
def _is_legal_split_op(op_idx, node_info, used_ops=None):
    """Legal split member: not unsplittable, not merge, not previously split."""
    used_ops = used_ops or set()
    if op_idx in used_ops:
        return False
    op_type = node_info[op_idx].get('op_type')
    return (op_type not in unsplittable_ops) and (op_type not in merging_ops)


def _is_boundary_end_op(op_idx, node_info):
    """Ops that may appear only as the end boundary of g."""
    op_type = node_info[op_idx].get('op_type')
    return (op_type in unsplittable_ops) or (op_type in merging_ops)


def _q_checked_ops(g_ops, node_info, used_ops=None):
    """
    q is checked only for real split operators.
    Boundary-end ops such as Concat/Add/Reshape are excluded from q validation.
    """
    return [op for op in g_ops if _is_legal_split_op(op, node_info, used_ops)]


def _valid_q_for_ops(g_ops, node_info, valid_q=None):
    """Return duplication/split factors valid for every real split op in one g candidate."""
    valid_q = valid_q or VALID_Q
    if not g_ops:
        return []
    ...
    for q in valid_q:
        ...
        for op in g_ops:
            node = node_info[op]
            ishapes = node.get('input_shapes') or []
            oshapes = node.get('output_shapes') or []
            if (not ishapes or not oshapes or ishapes[0] is None or oshapes[0] is None
                    or len(ishapes[0]) < 3 or len(oshapes[0]) < 3):
                ok = False
                break
            h_in = ishapes[0][2]
            h_out = oshapes[0][2]
            if (h_in is None or h_out is None or
                    h_in < q or h_out < q or
                    h_in % q != 0 or h_out % q != 0 or
                    h_in // q <= 1 or h_out // q <= 1):
                ok = False
                break
        if ok:
            q_candidates.append(q)
...
def _iter_branch_sets(total_sets_of_branches):
    """Flatten branch sets, including nested sub_branches."""
    for br in total_sets_of_branches or []:
        yield br
        for child in br.get('sub_branches', []) or []:
            yield from _iter_branch_sets([child])


def _unique_ops_keep_order(ops):
    seen = set()
    out = []
    for op in ops:
        if op not in seen:
...
def _producer_ops_of_input_tensors(op_idx, node_info, data_usage):
    """Return producer ops of all input tensors of op_idx using data_usage[t]['from']."""
    producers = []
    for tid in node_info[op_idx].get('data_in', []) or []:
        for p in data_usage[tid].get('from', []) or []:
            if p is not None and p not in producers:
                producers.append(p)
    return producers


def _consumer_ops_of_output_tensors(op_idx, node_info, data_usage):
    """Return consumer ops of all output tensors of op_idx using data_usage[t]['to']."""
    consumers = []
    for tid in node_info[op_idx].get('data_out', []) or []:
        for c in data_usage[tid].get('to', []) or []:
            if c is not None and c not in consumers:
                consumers.append(c)
    return consumers


def _backward_linear_paths_to_op(end_op, node_info, data_usage, used_ops=None, max_depth=64):
    """
    Generate graph-based suffix paths ending at end_op.
    Example: if A -> B -> end_op, return [end_op], [B,end_op], [A,B,end_op].
    Stop when the predecessor is not a legal split op, has multiple predecessors, or creates a cycle.
    """
    used_ops = used_ops or set()
    if not _is_legal_split_op(end_op, node_info, used_ops):
        return []
    ...
    for _ in range(max_depth):
        preds = _producer_ops_of_input_tensors(curr, node_info, data_usage)
        legal_preds = [p for p in preds if _is_legal_split_op(p, node_info, used_ops)]
    ...
    # all suffixes ending at end_op
    return [path[i:] for i in range(len(path))]


def _single_legal_predecessor(op_idx, node_info, data_usage, used_ops=None):
    """
    Return the single legal predecessor through input tensors, or None.

    This follows ONNX tensor producer links, not numeric op indices.
    Expansion stops if there are 0 or >1 legal predecessors.
    """
    used_ops = used_ops or set()
    preds = _producer_ops_of_input_tensors(op_idx, node_info, data_usage)
    legal_preds = [p for p in preds if _is_legal_split_op(p, node_info, used_ops)]
    if len(legal_preds) == 1:
...
def _single_legal_successor(op_idx, node_info, data_usage, used_ops=None):
    """
    Return the single legal successor through output tensors, or None.

    This follows ONNX tensor consumer links, not numeric op indices.
    Expansion stops if there are 0 or >1 legal successors.
    """
    used_ops = used_ops or set()
    succs = _consumer_ops_of_output_tensors(op_idx, node_info, data_usage)
    legal_succs = [s for s in succs if _is_legal_split_op(s, node_info, used_ops)]
    if len(legal_succs) == 1:
        return legal_succs[0]
    return None


def _linear_connected_span_around_bottleneck(peak_op, node_info, data_usage, used_ops=None, max_depth=128):
    """
    Build the maximal single-path connected legal span around bottleneck.

    Example by topology, not numeric order:
        21 -> 22 -> 23 -> 24 -> 25 -> 26 -> 27

    If peak_op=25 and ops 20 and 28 are unsplittable or unavailable,
    return:
        [21,22,23,24,25,26,27]

    The real model uses ONNX tensor producer/consumer links.
    """
    used_ops = used_ops or set()
    if not _is_legal_split_op(peak_op, node_info, used_ops):
        return []
    ...
    for _ in range(max_depth):
        p = _single_legal_predecessor(curr, node_info, data_usage, used_ops)
    ...
    for _ in range(max_depth):
        s = _single_legal_successor(curr, node_info, data_usage, used_ops)
    ...
    return left + [peak_op] + right


def _enumerate_connected_subpaths_containing_bottleneck(span_ops, bottleneck_op):
    """
    Enumerate all connected g candidates within a linear span, and every g
    must contain bottleneck_op.

    Example:
        span=[21,22,23,24,25,26,27], bottleneck=25

    Candidates include:
        [24,25], [25,26], [24,25,26],
        [23,24,25], [23,24,25,26], ... [21,...,27]

    Exclude degenerate [25] because random TS g must contain at least 2 ops.
    """
    if not span_ops or bottleneck_op not in span_ops:
        return []

    bpos = span_ops.index(bottleneck_op)
    candidates = []

    for i in range(0, bpos + 1):
        for j in range(bpos, len(span_ops)):
            g = list(span_ops[i:j + 1])
            if len(g) < 2:
                continue
            candidates.append(g)
...
def _branch_set_cover_ops(branch_set):
    """Return a stable op list covering shared input, internal operations, and merge."""
    shared = branch_set.get('shared_inputs_from')
    merge = branch_set.get('merged_by')
    ops = list(branch_set.get('operations', []) or [])
    ...
    if merge is not None:
        out.append(merge)
    return _unique_ops_keep_order(out)


def _branch_paths_from_branch_set(branch_set, node_info, used_ops=None, include_boundary_end=True):
    """
    Split branch-set operations into per-branch legal paths according to br_num.
    Boundary merge can be appended as final end if include_boundary_end=True.
    """
    used_ops = used_ops or set()
    ops = list(branch_set.get('operations', []) or [])
    br_heads = sorted(branch_set.get('br_num', []) or [])
    merge = branch_set.get('merged_by')

    branch_paths = []
    if not ops or not br_heads:
        return branch_paths

    for i, head in enumerate(br_heads):
        next_head = br_heads[i + 1] if i + 1 < len(br_heads) else merge
        br_ops = [
            op for op in ops
            if head <= op < next_head and _is_legal_split_op(op, node_info, used_ops)
        ]
        if br_ops:
            if include_boundary_end and merge is not None:
                branch_paths.append(br_ops + [merge])
            else:
                branch_paths.append(br_ops)

    return branch_paths


def _branch_related_g_candidates_for_peak(peak_op, node_info, data_usage, total_sets_of_branches, used_ops=None):
    """
    Additional branch-related g candidates when the bottleneck is inside or near
    a branch set.

    If the candidate includes a shared input or merge boundary, generate options
    that include 1, 2, ..., all related branches. This keeps the random-search
    space aware of branch structures.
    """
    used_ops = used_ops or set()
    candidates = []
    ...
    for br in _iter_branch_sets(total_sets_of_branches):
        shared = br.get('shared_inputs_from')
        merge = br.get('merged_by')
        cover = set(_branch_set_cover_ops(br))

        if peak_op not in cover:
            continue

        branch_paths = _branch_paths_from_branch_set(
            br, node_info, used_ops=used_ops, include_boundary_end=False
        )
        if not branch_paths:
            continue

        # Branch subsets: 1, 2, ..., all branches.
        for r in range(1, len(branch_paths) + 1):
            for subset in combinations(branch_paths, r):
                # For each branch path, allow suffixes/prefixes that keep peak if peak is in that branch.
                options_per_branch = []
                skip_subset = False

                for path in subset:
                    if peak_op in path:
                        bpos = path.index(peak_op)
                        opts = []
                        for i in range(0, bpos + 1):
                            for j in range(bpos, len(path)):
                                g = path[i:j + 1]
                                if len(g) >= 2:
                                    opts.append(g)
                        if not opts:
                            skip_subset = True
                            break
                        options_per_branch.append(opts)
                    else:
                        # If peak is at shared/merge or another selected branch, this whole branch path is an option.
                        options_per_branch.append([path])

                if skip_subset:
                    continue

                for chosen_parts in product(*options_per_branch):
                    flat = []

                    # If all branches are selected and shared is legal, include shared as boundary/start option.
                    if r == len(branch_paths) and shared is not None and _is_legal_split_op(shared, node_info, used_ops):
                        flat.append(shared)

                    for part in chosen_parts:
                        flat.extend(part)

                    # If merge is the bottleneck or all branches are selected, include merge as boundary end.
                    if merge is not None and (peak_op == merge or r == len(branch_paths)):
                        flat.append(merge)

                    flat = _unique_ops_keep_order(flat)
                    if peak_op in flat and len(flat) >= 2:
                        candidates.append(flat)

    return candidates


def _all_legal_g_ops_for_normal_peak(peak_op, node_info, data_usage, used_ops=None):
    """
    Bottleneck-centered g candidates for a normal splittable bottleneck.

    Correct rule:
      - start from the bottleneck
      - expand left/right through ONNX input/output tensor connections
      - stop when the next neighbor is unsplittable, merge, reused, absent, or ambiguous
      - enumerate every connected g within the maximal span that contains bottleneck
      - exclude g=[bottleneck]
    """
    used_ops = used_ops or set()

    span_ops = _linear_connected_span_around_bottleneck(
        peak_op, node_info, data_usage, used_ops=used_ops
    )
    return _enumerate_connected_subpaths_containing_bottleneck(span_ops, peak_op)


def _branch_ops_from_branch_set_to_merge(merge_op, node_info, used_ops, branch_set):
    """
    Extra candidates from the identified branch-set metadata.
    Allows all-branch g where the shared input can be included when every branch is included.
    """
    candidates = []
    if branch_set.get('merged_by') != merge_op:
        return candidates

    ops = branch_set.get('operations', []) or []
    br_heads = sorted(branch_set.get('br_num', []) or [])
    if not ops or not br_heads:
        return candidates

    # Split the branch-set operations into branch paths according to br_num boundaries.
    branch_paths = []
    for i, head in enumerate(br_heads):
        next_head = br_heads[i + 1] if i + 1 < len(br_heads) else merge_op
        br_ops = [op for op in ops if head <= op < next_head and _is_legal_split_op(op, node_info, used_ops)]
        if br_ops:
            branch_paths.append(br_ops)

    # one branch, multiple branches, all branches
    from itertools import combinations, product
    for r in range(1, len(branch_paths) + 1):
        for subset in combinations(branch_paths, r):
            # For each selected branch, allow starting from any op on that branch.
            suffix_options = []
            for br in subset:
                suffix_options.append([br[i:] for i in range(len(br))])

            for selected_suffixes in product(*suffix_options):
                flat = []
                for suffix in selected_suffixes:
                    flat.extend(suffix)

                # If all branches are included, optionally include the shared input as start.
                if r == len(branch_paths):
                    shared = branch_set.get('shared_inputs_from')
                    if shared is not None and _is_legal_split_op(shared, node_info, used_ops):
                        flat = [shared] + flat

                flat.append(merge_op)  # merge op is boundary end only
                candidates.append(_unique_ops_keep_order(flat))

    return candidates


def _all_legal_g_ops_for_merge_peak(peak_op, node_info, data_usage, total_sets_of_branches, used_ops=None):
    """
    Graph-based g candidates for a merge/unsplittable bottleneck.
    The peak_op is used as boundary end; starts come from one or more input branches.
    """
    used_ops = used_ops or set()
    candidates = []

    # 1) Use tensor producer links: each input producer can be the branch end.
    producers = _producer_ops_of_input_tensors(peak_op, node_info, data_usage)
    branch_options = []
    for p in producers:
        opts = _backward_linear_paths_to_op(p, node_info, data_usage, used_ops)
        opts = [path + [peak_op] for path in opts if path]
        if opts:
            branch_options.append(opts)

    # one input branch, multiple input branches, all input branches
    from itertools import combinations, product
    for r in range(1, len(branch_options) + 1):
        for subset in combinations(branch_options, r):
            for chosen in product(*subset):
                flat = []
                for path in chosen:
                    flat.extend(path)
                candidates.append(_unique_ops_keep_order(flat))

    # 2) Also use branch-set metadata to include all-branch candidates from shared input.
    for br in _iter_branch_sets(total_sets_of_branches):
        candidates.extend(_branch_ops_from_branch_set_to_merge(peak_op, node_info, used_ops, br))

    # 3) Deduplicate and keep candidates with at least one real split op.
    dedup = []
    seen = set()
    for g_ops in candidates:
        g_ops = _unique_ops_keep_order(g_ops)
        if not _q_checked_ops(g_ops, node_info, used_ops):
            continue
        key = tuple(g_ops)
        if key not in seen:
            seen.add(key)
...
def _all_legal_g_ops_for_peak(peak_op, node_info, data_usage, total_sets_of_branches, used_ops=None):
    """
    Return graph-based g candidates as op lists, not (start,end) continuous ranges.
    - normal peak: g contains the peak op.
    - merge/unsplittable peak: peak op may appear only as boundary end.
    """
    if _is_legal_split_op(peak_op, node_info, used_ops):
        candidates = []
        candidates.extend(_all_legal_g_ops_for_normal_peak(peak_op, node_info, data_usage, used_ops))
        candidates.extend(_branch_related_g_candidates_for_peak(
            peak_op, node_info, data_usage, total_sets_of_branches, used_ops
        ))

        dedup = []
        seen = set()
        for g in candidates:
            g = _unique_ops_keep_order(g)
            if peak_op not in g or len(g) < 2:
                continue
            key = tuple(g)
            if key not in seen:
                seen.add(key)
                dedup.append(g)
        return dedup
    if _is_boundary_end_op(peak_op, node_info):
        return _all_legal_g_ops_for_merge_peak(
            peak_op, node_info, data_usage, total_sets_of_branches, used_ops
        )
    return []


def _schedule_candidates_for_g(g_ops, node_info, total_sets_of_branches, max_materialize=200):
    """
    Prefer path schedules when the g range intersects branch paths.
    Fallback to one operator schedule when no path-order permutation exists.
    """
    branch_info, _ = get_subgraph_branch_info(g_ops, list(_iter_branch_sets(total_sets_of_branches)), node_info)
    before_all, between_sets, after_all = split_by_branch_sets(g_ops, branch_info)

    # No branch-path choices: fallback to the normal operator order.
    if not branch_info:
        return {
            'mode': 'operator',
            'num': 1,
            'materialized': [list(g_ops)],
            'note': 'fallback operator schedule'
        }

    # Exact count without always materializing every order.
    num = 1
    for _, branches in branch_info.items():
        num *= math.factorial(len(branches))

    materialized = []
    if num <= max_materialize:
        materialized = generate_all_subgraph_orders(before_all, between_sets, after_all, branch_info)

    return {
        'mode': 'path',
        'num': num,
        'materialized': materialized,
        'note': 'path schedule by branch permutation'
    }


def _g_option_matches_exgrule(rule_check, exgrule):
    """Return True if this g option belongs to the requested excluded-rule pool.

    shared_branch_rule_status has three meanings:
      - "true":   g includes a real shared node or merge node, and all related branches are included.
      - "false":  g includes a real shared node or merge node, but not all related branches are included.
      - "ignore": g does not include a real shared node or merge node, so branch completeness is not applicable.

    New --exgrule names:
      - neither: boundary_rule=False and shared_branch_rule_status in ('false', 'ignore')
      - PConly:  boundary_rule=False and shared_branch_rule_status in ('true', 'ignore')
      - BPonly:  boundary_rule=True  and shared_branch_rule_status in ('false', 'ignore')
      - one:     boundary_rule=True  and shared_branch_rule_status in ('true', 'ignore')

    Backward-compatible aliases:
      - both -> neither, boundary -> PConly, branches -> BPonly
    """
    exgrule = {"both": "neither", "boundary": "PConly", "branches": "BPonly"}.get(exgrule, exgrule)
    boundary_ok = bool(rule_check.get('boundary_rule_ok'))

    status = rule_check.get('shared_branch_rule_status', None)
    if status is None:
        # Backward compatibility for older records that only have shared_branch_rule_ok.
        status = 'true' if bool(rule_check.get('shared_branch_rule_ok')) else 'false'
    status = str(status).lower()

    if exgrule == 'neither':
        return (not boundary_ok) and (status in ('false', 'ignore'))
    if exgrule == 'PConly':
        return (not boundary_ok) and (status in ('true', 'ignore'))
    if exgrule == 'BPonly':
        return boundary_ok and (status in ('false', 'ignore'))
    if exgrule == 'one':
        return boundary_ok and (status in ('true', 'ignore'))

    raise ValueError(f"Unsupported exgrule: {exgrule}")


def _remove_g_option_by_rule_filter(pair_info, bottleneck_op, node_info, data_usage,
                                    total_sets_of_branches, exgrule='both'):
    """
    User-requested random-sampling filter.

    Keep only the g options selected by --exgrule:
      --exgrule both      => boundary_rule=False and shared_branch_rule_status="false"
      --exgrule boundary  => boundary_rule=False and shared_branch_rule_status in {"true", "ignore"}
      --exgrule branches  => boundary_rule=True  and shared_branch_rule_status in {"false", "ignore"}
      --exgrule one       => boundary OR branches

    Removed options are stored for debug reporting and cannot be sampled later.
    """
    g_ops = list(pair_info.get('ops', []) or [])
    rule_check = _check_g_selection_rules(
        g_ops, bottleneck_op, node_info, data_usage, total_sets_of_branches
    )
    keep = _g_option_matches_exgrule(rule_check, exgrule)
    return (not keep), rule_check


def count_random_search_combinations(node_info, data_usage, mem_usage, total_sets_of_branches,
                                     used_ops=None, max_materialize_schedules=200, exgrule='both'):
    """
    For each bottleneck over VM, count combinations of:
      1) pair-expanded g candidate,
      2) duplication factor q,
      3) schedule choices.

    Raw graph-based g candidates are first generated, then each raw g is expanded
    into all valid (start_op, end_op) boundary pairs around the bottleneck.

    Example:
        raw_g_ops = [56, 57, 58, 59, 60]
        bottleneck_op = 58

    Tested pairs:
        (56,58), (57,58), (56,59), (57,59), (56,60), (57,60)

    Kept only if:
        input(start_op) < input(bottleneck_op)
        output(end_op)  < input(bottleneck_op)
    """
    used_ops = set(used_ops or [])
    report = []
    bottlenecks = mem_usage.get('over_mem_idx', [])

    for peak_op in bottlenecks:
        raw_g_ops_list = _all_legal_g_ops_for_peak(
            peak_op, node_info, data_usage, total_sets_of_branches, used_ops
        )
        total_combinations = 0
        g_details = []
        seen_final_g_ops = set()  # avoid counting the same final g multiple times from different source_g
        dup_factor_union = set()
        schedule_modes = set()
        removed_by_rule_filter = []

        for raw_g_ops in raw_g_ops_list:
            pair_infos = _boundary_pair_candidates_for_g(
                raw_g_ops,
                peak_op,
                node_info,
                data_usage,
                used_ops=used_ops,
                require_smaller_than_bottleneck=False,
            )

            for pair_info in pair_infos:
                g_ops = pair_info['ops']

                remove_g_option, rule_check_for_filter = _remove_g_option_by_rule_filter(
                    pair_info, peak_op, node_info, data_usage, total_sets_of_branches, exgrule=exgrule
                )
                if remove_g_option:
                    removed_by_rule_filter.append({
                        'ops': list(g_ops),
                        'source_g_ops': list(raw_g_ops),
                        'boundary_pair': (pair_info.get('start_op'), pair_info.get('end_op')),
                        'bottleneck_input_size': pair_info.get('bottleneck_size'),
                        'start_in': pair_info.get('start_in'),
                        'end_out': pair_info.get('end_out'),
                        'boundary_rule_ok': rule_check_for_filter.get('boundary_rule_ok'),
                        'shared_branch_rule_ok': rule_check_for_filter.get('shared_branch_rule_ok'),
                        'shared_branch_rule_status': rule_check_for_filter.get('shared_branch_rule_status'),
                        'boundary_rule_msg': rule_check_for_filter.get('boundary_rule_msg'),
                        'shared_branch_rule_msg': rule_check_for_filter.get('shared_branch_rule_msg'),
                        'reason': f'removed because it does not match --exgrule {exgrule}',
                    })
                    continue

                # The same final g can be produced from multiple larger source_g ranges.
                # Count/sample each final TS config only once.
                # Example:
                #   source_g=[24,25]       -> final g=[24,25]
                #   source_g=[24,25,26]    -> final g=[24,25]
                #   source_g=[24,25,26,27] -> final g=[24,25]
                # These are the same TS setting and must not be repeated.
                g_key = tuple(g_ops)
                if g_key in seen_final_g_ops:
                    continue
                seen_final_g_ops.add(g_key)

                q_ops = _q_checked_ops(g_ops, node_info, used_ops)
                q_list = _valid_q_for_ops(q_ops, node_info, VALID_Q)
                if not q_list:
                    continue

                sched_info = _schedule_candidates_for_g(
                    g_ops, node_info, total_sets_of_branches,
                    max_materialize=max_materialize_schedules
                )
                comb = len(q_list) * sched_info['num']
                total_combinations += comb
                dup_factor_union.update(q_list)
                schedule_modes.add(sched_info['mode'])

                g_details.append({
                    'g': (g_ops[0], g_ops[-1]),
                    'ops': g_ops,
                    'source_g_ops': raw_g_ops,
                    'boundary_pair': (pair_info['start_op'], pair_info['end_op']),
                    'bottleneck_input_size': pair_info['bottleneck_size'],
                    'start_in': pair_info['start_in'],
                    'end_out': pair_info['end_out'],
                    'boundary_pair_rule_ok': pair_info.get('boundary_pair_rule_ok', None),
                    # Final rule-filter check stored for debugging/reporting.
                    # Any option with boundary_rule_ok=True OR shared_branch_rule_ok=True
                    # must not be sampled.
                    'boundary_rule_ok': rule_check_for_filter.get('boundary_rule_ok'),
                    'shared_branch_rule_ok': rule_check_for_filter.get('shared_branch_rule_ok'),
                    'shared_branch_rule_status': rule_check_for_filter.get('shared_branch_rule_status'),
                    ...
                    'q_checked_ops': q_ops,
                    'q_candidates': q_list,
                    'schedule_mode': sched_info['mode'],
                    'schedule_num': sched_info['num'],
                    'schedule_samples': sched_info['materialized'][:3],
                    'combination_num': comb,
                })

        report.append({
            'exgrule': exgrule,
            'bottleneck_op': peak_op,
            'op_type': node_info[peak_op].get('op_type'),
            'memory_bytes': mem_usage['total'][peak_op],
            'memory_kb': mem_usage['total'][peak_op] / 1024,
            'g_ranges': [tuple(gd['ops']) for gd in g_details],
            'duplication_factors_union': sorted(dup_factor_union),
            'schedule_modes': sorted(schedule_modes),
            'total_g_ranges': len(g_details),
            'removed_g_ranges_by_rule_filter': len(removed_by_rule_filter),
            'removed_g_details_by_rule_filter': removed_by_rule_filter[:20],
            'has_no_g_options_after_filter': (len(g_details) == 0),
...
def _model_level_combination_product(report):
    """
    Product of per-bottleneck combination counts before non-overlap filtering.

    If a bottleneck has 0 valid g options after filtering, use factor=1
    instead of factor=0. This keeps the reported product from becoming 0
    when some bottlenecks are unresolved/no-g. The number of such bottlenecks
    is reported separately by _zero_valid_g_bottleneck_count(...).
    """
    prod_val = 1
    for item in report:
        comb = int(item.get('total_combinations', 0))
        prod_val *= max(1, comb)
    return prod_val


def _zero_valid_g_bottleneck_count(report):
    """Count bottlenecks with 0 valid g options after rule filtering."""
    return sum(
        1
        for item in report
        if int(item.get('total_g_ranges', 0)) == 0
        or int(item.get('total_combinations', 0)) == 0
        or bool(item.get('has_no_g_options_after_filter', False))
    )


def _op_io_size(op, node_info, data_usage):
    """Sum input + output tensor bytes for an op."""
    ins = sum(data_usage[tid].get('size', 0) for tid in node_info[op].get('data_in', []))
    outs = sum(data_usage[tid].get('size', 0) for tid in node_info[op].get('data_out', []))
    return ins + outs


def _op_input_size(op, node_info, data_usage):
    return sum(data_usage[tid].get('size', 0) for tid in node_info[op].get('data_in', []))


def _op_output_size(op, node_info, data_usage):
    return sum(data_usage[tid].get('size', 0) for tid in node_info[op].get('data_out', []))


def _slice_ops_between_positions(g_ops, start_op, end_op):
    """
    Return the sub-list of g_ops from start_op to end_op, based on position in g_ops.
    This works even if g_ops is graph-based and not strictly continuous by index.
    """
    try:
        i = g_ops.index(start_op)
        j = g_ops.index(end_op)
    except ValueError:
        return []
    ...
    return list(g_ops[i:j + 1])


def _boundary_pair_candidates_for_g(
    g_ops,
    bottleneck_op,
    node_info,
    data_usage,
    ...
    """
    Convert one connected raw g candidate into one sampled g candidate.

    Important:
        The final sampled g should be the whole connected g_ops, not a sliced
        sub-pair from inside g_ops.

    Example:
        source_g = [24, 25, 26, 27]
        bottleneck = 25
    ...
    The raw g generator is already responsible for producing smaller/larger
    connected options. This function only attaches boundary information and
    evaluates whether the g-rule is satisfied.
    """
    used_ops = used_ops or set()
    ...
    g_ops = _unique_ops_keep_order(list(g_ops))

    # Random/sample g must contain at least two operators.
    if len(g_ops) < 2:
        return []

    if bottleneck_op not in g_ops:
        return []

    start_op = g_ops[0]
    end_op = g_ops[-1]

    # All internal ops before the final boundary end must be legal split ops.
    # The final end op may be a boundary op, such as merge/unsplittable.
    for op in g_ops[:-1]:
        if not _is_legal_split_op(op, node_info, used_ops):
            return []

    split_ops = _q_checked_ops(g_ops, node_info, used_ops)
    if not split_ops:
        return []

    bottleneck_size = _op_input_size(bottleneck_op, node_info, data_usage)
    start_in = _op_input_size(start_op, node_info, data_usage)
    end_out = _op_output_size(end_op, node_info, data_usage)

    # g-rule analysis only. Do not drop candidates in random sampling unless
    # strict filtering is explicitly requested.
    boundary_rule_ok = (
        bottleneck_size > 0
        and start_op != bottleneck_op
        and g_ops.index(start_op) < g_ops.index(bottleneck_op)
        and g_ops.index(end_op) >= g_ops.index(bottleneck_op)
        and start_in < bottleneck_size
        and end_out < bottleneck_size
    )

    if require_smaller_than_bottleneck and not boundary_rule_ok:
        return []
    ...
    return [{
        ...
        'boundary_pair_rule_ok': boundary_rule_ok,
    }]


def _check_boundary_smaller_rule(g_ops, bottleneck_op, node_info, data_usage):
    """
    Rule for one selected pair-expanded g:
    ...
    """
    if not g_ops:
        return False, 'empty g_ops'

    if bottleneck_op not in g_ops:
        return False, f'bottleneck_op={bottleneck_op} not in g_ops={g_ops}'

    start_op = g_ops[0]
    end_op = g_ops[-1]

    if start_op == bottleneck_op:
        return False, f'start_op={start_op} is bottleneck; need start before bottleneck'

    if g_ops.index(start_op) > g_ops.index(bottleneck_op):
        return False, f'start_op={start_op} appears after bottleneck_op={bottleneck_op}'

    if g_ops.index(end_op) < g_ops.index(bottleneck_op):
        return False, f'end_op={end_op} appears before bottleneck_op={bottleneck_op}'

    if not _is_legal_split_op(start_op, node_info, used_ops=set()):
        return False, f'start_op={start_op} is not a legal split op'

    # Every op before the final end must be legal split op.
    # The final end itself may be a boundary op such as Concat/Add/Reshape.
    for op in g_ops[:-1]:
        if not _is_legal_split_op(op, node_info, used_ops=set()):
            return False, f'op={op} appears before end but is not legal split op'

    bottleneck_size = _op_input_size(bottleneck_op, node_info, data_usage)
    start_in = _op_input_size(start_op, node_info, data_usage)
    end_out = _op_output_size(end_op, node_info, data_usage)

    if not (start_in < bottleneck_size and end_out < bottleneck_size):
        return (
            False,
            f'boundary tensors not smaller than bottleneck input: '
            f'start={start_op}, end={end_op}, bottleneck={bottleneck_op}, '
            f'start_in={start_in}, end_out={end_out}, bottleneck_input={bottleneck_size}'
        )

    return (
        True,
        f'boundary rule ok: start={start_op}, end={end_op}, bottleneck={bottleneck_op}, '
        f'start_in={start_in}, end_out={end_out}, bottleneck_input={bottleneck_size}'
    )


def _ops_required_for_complete_branch_set(br):
    """
    Required ops when g touches a branch boundary.

    If g contains the shared-input node or merge node of a branch set,
    then the g should include:
      - shared input node
      - all branch internal operators from all branches
      - merge node

    Example:
      shared node has 4 outputs to 4 branches and merge has 4 inputs.
      If g contains shared or merge, but misses any op from any branch,
      shared_branch_rule_ok=False.
    """
    required = []
    shared = br.get('shared_inputs_from')
    merge = br.get('merged_by')
    ...
    required.extend(list(br.get('operations', []) or []))

    if merge is not None:
        required.append(merge)

    return _unique_ops_keep_order(required)


def _node_field(node_info, op_idx, key, default=None):
    """Read node_info[op_idx][key] safely when entries may be dicts or legacy lists."""
    if default is None:
        default = []
    if node_info is None or op_idx is None or op_idx < 0 or op_idx >= len(node_info):
        return default
    n = node_info[op_idx]
...
def _node_op_type(node_info, op_idx):
    if node_info is None or op_idx is None or op_idx < 0 or op_idx >= len(node_info):
        return None
    n = node_info[op_idx]
    if isinstance(n, dict):
        return n.get('op_type')
    return None


# Caches for graph-derived branch rule checks.
# Without these caches, random-count setup repeatedly scans node_info and recomputes
# reachability for every candidate g option, which can dominate runtime after rule filtering.
_GRAPH_SUCC_CACHE = {}
_GRAPH_REACH_CACHE = {}
_GRAPH_FIRST_MERGE_CACHE = {}
_GRAPH_PATH_OPS_CACHE = {}

def _graph_cache_key(node_info, *parts):
    return (id(node_info),) + tuple(parts)


def _op_successor_count_from_node_info(op_idx, node_info):
    """Count graph successors by matching this op's output tensors to later inputs."""
    if op_idx is None or node_info is None or op_idx < 0 or op_idx >= len(node_info):
        return 0
    out_tensors = set(_node_field(node_info, op_idx, 'data_out', []) or [])
    if not out_tensors:
        return 0
    succ = set()
    for j, n in enumerate(node_info):
        if j == op_idx:
            continue
        if isinstance(n, dict) and (out_tensors & set(n.get('data_in', []) or [])):
            succ.add(j)
    return len(succ)


def _is_real_shared_node(op_idx, br, node_info):
    """A shared node is real only when it actually fans out to >=2 branch heads."""
    if op_idx is None:
        return False
    br_heads = br.get('br_num', []) or []
    if len(br_heads) < 2:
        return False
    if node_info is None:
        return True
    return _op_successor_count_from_node_info(op_idx, node_info) >= 2


def _is_real_merge_node(op_idx, br, node_info):
    """A merge node is real only when it really joins >=2 branch inputs."""
    if op_idx is None:
        return False
    br_heads = br.get('br_num', []) or []
    if len(br_heads) < 2:
        return False
    if node_info is None or op_idx < 0 or op_idx >= len(node_info):
        return True
    return (_node_op_type(node_info, op_idx) in merging_ops) or (len(_node_field(node_info, op_idx, 'data_in', []) or []) >= 2)


def _graph_successors_from_node_info(op_idx, node_info):
    """Return direct successor op indices from node_info tensor connectivity, cached."""
    key = _graph_cache_key(node_info, 'succ', op_idx)
    if key in _GRAPH_SUCC_CACHE:
        return list(_GRAPH_SUCC_CACHE[key])

    if op_idx is None or node_info is None or op_idx < 0 or op_idx >= len(node_info):
        _GRAPH_SUCC_CACHE[key] = tuple()
        return []
    ...
    ans = tuple(sorted(set(succ)))
    _GRAPH_SUCC_CACHE[key] = ans
    return list(ans)

def _graph_reachable_from(start_op, node_info, max_depth=200):
    """Return all downstream nodes reachable from start_op, excluding start_op, cached."""
    key = _graph_cache_key(node_info, 'reach', start_op, max_depth)
    if key in _GRAPH_REACH_CACHE:
        return set(_GRAPH_REACH_CACHE[key])

    seen = set()
    stack = list(_graph_successors_from_node_info(start_op, node_info))
    depth = 0
    while stack and depth < max_depth:
        ...
        if cur in seen:
            continue
        seen.add(cur)
        stack.extend(_graph_successors_from_node_info(cur, node_info))
        depth += 1

    _GRAPH_REACH_CACHE[key] = frozenset(seen)
    return set(seen)

def _first_graph_merge_after_shared(shared, node_info):
    """
    Find the first actual downstream merge for a shared/fan-out node using graph
    connectivity, not only the branch metadata. Cached because this is called for
    many candidate g options from the same shared node.
    """
    key = _graph_cache_key(node_info, 'first_merge', shared)
    if key in _GRAPH_FIRST_MERGE_CACHE:
        return _GRAPH_FIRST_MERGE_CACHE[key]

    succs = _graph_successors_from_node_info(shared, node_info)
    if len(succs) < 2:
        _GRAPH_FIRST_MERGE_CACHE[key] = None
        return None

    reachable_sets = []
    for s in succs:
        reachable_sets.append({s} | _graph_reachable_from(s, node_info))
    ...
    for op in common:
        if op <= shared:
            continue
        if (_node_op_type(node_info, op) in merging_ops) or (len(_node_field(node_info, op, 'data_in', []) or []) >= 2):
            merge_like.append(op)

    if merge_like:
        ans = min(merge_like)
    else:
        candidates = [op for op in common if op > shared]
        ans = min(candidates) if candidates else None

    _GRAPH_FIRST_MERGE_CACHE[key] = ans
    return ans

def _ops_on_paths_between_shared_and_merge(shared, merge, node_info):
    """Return all ops on any path from shared to merge, including both endpoints, cached."""
    key = _graph_cache_key(node_info, 'path_ops', shared, merge)
    if key in _GRAPH_PATH_OPS_CACHE:
        return list(_GRAPH_PATH_OPS_CACHE[key])

    if shared is None or merge is None:
        _GRAPH_PATH_OPS_CACHE[key] = tuple()
        return []

    required = {shared, merge}
    stack = list(_graph_successors_from_node_info(shared, node_info))
    seen = set()
    while stack:
        cur = stack.pop()
        if cur in seen:
        ...
        if cur > merge:
            continue
        # Keep only nodes that can still reach merge. This avoids unrelated fan-out tails.
        if cur != merge and merge not in _graph_reachable_from(cur, node_info):
            continue
        required.add(cur)
        if cur != merge:
            stack.extend(_graph_successors_from_node_info(cur, node_info))

    ans = tuple(sorted(required))
    _GRAPH_PATH_OPS_CACHE[key] = ans
    return list(ans)

def _check_shared_input_branch_rule(g_ops, total_sets_of_branches, node_info=None):
    """
    Three-state branch-completeness rule for reporting/filtering.

    Returns:
      (ok_bool, message, status)

    status meanings:
      - "true":
          g includes a real shared node or real merge node, and the complete related
          branch region is included.
      - "false":
          g includes a real shared node or real merge node, but misses at least one
          related branch operator/boundary.
      - "ignore":
          g does not include a real shared node or real merge node, so this branch
          rule is not applicable.

    Note: "ignore" is treated as OK for rule checking, because there is no branch
    completeness condition to enforce when no shared/merge boundary is touched.
    """
    g_set = set(g_ops or [])
    touched_any_real_shared_or_merge = False

    for br in _iter_branch_sets(total_sets_of_branches):
        shared = br.get('shared_inputs_from')
        metadata_merge = br.get('merged_by')

        real_shared = shared if _is_real_shared_node(shared, br, node_info) else None
        real_merge = metadata_merge if _is_real_merge_node(metadata_merge, br, node_info) else None

        touched_shared = (real_shared is not None and real_shared in g_set)
        touched_merge = (real_merge is not None and real_merge in g_set)

        if not (touched_shared or touched_merge):
            continue

        touched_any_real_shared_or_merge = True

        # Prefer graph-derived merge when available, otherwise use branch metadata.
        graph_merge = _first_graph_merge_after_shared(real_shared, node_info) if (real_shared is not None and node_info is not None) else None
        merge = graph_merge if graph_merge is not None else metadata_merge

        if real_shared is not None and merge is not None and node_info is not None:
            required_ops = _ops_on_paths_between_shared_and_merge(real_shared, merge, node_info)
        else:
            required_ops = []

        if not required_ops:
            required_ops = _ops_required_for_complete_branch_set(br)

        missing_ops = [op for op in required_ops if op not in g_set]
        if missing_ops:
            touched_desc = []
            if touched_shared:
                touched_desc.append(f'shared={real_shared}')
            if touched_merge:
                touched_desc.append(f'merge={real_merge}')
            return (
                False,
                f'g includes {" and ".join(touched_desc)}, but does not include all related branches; '
                f'required_ops={required_ops}, missing_ops={missing_ops}',
                'false'
            )

    if not touched_any_real_shared_or_merge:
        return (
            True,
            'shared-branch rule ignored: g does not include a real shared node or real merge node',
            'ignore'
        )

    return True, 'complete shared/merge branch set rule ok', 'true'


def _check_g_selection_rules(g_ops, bottleneck_op, node_info, data_usage, total_sets_of_branches):
    b_ok, b_msg = _check_boundary_smaller_rule(g_ops, bottleneck_op, node_info, data_usage)
    s_ok, s_msg, s_status = _check_shared_input_branch_rule(g_ops, total_sets_of_branches, node_info)
    # Branch status "ignore" should not make the overall rule fail.
    branch_ok_for_overall = (s_status != 'false')
    return {
        'ok': bool(b_ok and branch_ok_for_overall),
        'boundary_rule_ok': bool(b_ok),
        'boundary_rule_msg': b_msg,
        'shared_branch_rule_ok': bool(branch_ok_for_overall),
        'shared_branch_rule_status': s_status,
        'shared_branch_rule_msg': s_msg,
    }


def _sample_schedule_for_g(g_ops, node_info, total_sets_of_branches, rng):
    """Sample one schedule. Prefer path schedule if branch permutations exist; otherwise operator order."""
    branch_info, _ = get_subgraph_branch_info(g_ops, list(_iter_branch_sets(total_sets_of_branches)), node_info)
    if not branch_info:
        return list(g_ops), 'operator'

    before_all, between_sets, after_all = split_by_branch_sets(g_ops, branch_info)
    sorted_set_ids = sorted(branch_info.keys())
    order = list(before_all)
    for i, set_id in enumerate(sorted_set_ids):
        branches = list(branch_info[set_id].values())
        rng.shuffle(branches)
        for br in branches:
            order.extend(br)
        if i < len(sorted_set_ids) - 1:
            key = (sorted_set_ids[i], sorted_set_ids[i + 1])
            order.extend(between_sets.get(key, []))
    order.extend(after_all)
    return _unique_ops_keep_order(order), 'path'


def _safe_first_for_random(lst):
    return lst[0] if lst and len(lst) > 0 else None


def _tensor_size_for_random(tid, data_usage):
    if tid is None:
        return 0
    if not (0 <= int(tid) < len(data_usage)):
        return 0
    return int(data_usage[int(tid)].get("size", 0))


def estimate_peak_for_given_gsd(
    g_ops,
    schedule,
    q,
    node_info,
    ...
    """
    Estimate peak memory for one sampled (g, s, d/q) using the same DupNAS
    formulas as search_dxq_single_branch() and search_dxq().

    g:
        sampled connected operator set

    s:
        sampled operator/path schedule.
        For branch g, this determines ordered branch execution.

    d/q:
        tensor splitting factor.

    This function does NOT search for q or d. It only evaluates the sampled
    q and sampled schedule.
    """
    q = max(1, int(q))
    g_ops = list(g_ops or [])
    schedule = list(schedule or g_ops)

    if not g_ops or len(g_ops) < 2:
        return 0, []

    flat_branch_sets = list(_iter_branch_sets(total_sets_of_branches))
    branch_info, _ = get_subgraph_branch_info(g_ops, flat_branch_sets, node_info)

    if branch_info:
        return estimate_peak_for_given_branch_gsd(
            g_ops,
            schedule,
            q,
            node_info,
    ...
    return estimate_peak_for_given_single_gsd(
        g_ops,
        q,
        node_info,
        data_usage,
...
def estimate_peak_for_given_single_gsd(g_ops, q, node_info, data_usage, mem_usage):
    """
    Same formula as search_dxq_single_branch(), but q/g are fixed.

    Original DupNAS single-branch formula:
        start_ret = start_size // q * (q - a - 1)
        end_ret   = end_size   // q * a
        total      = ifm // q + ofm // q + start_ret + end_ret
    """
    ...
    start_size = _op_input_size(start, node_info, data_usage)
    end_size = _op_output_size(end, node_info, data_usage)
    ...
    for a in range(q):
        ...
        for op in best_d:
            ifm = _op_input_size(op, node_info, data_usage)
            ofm = _op_output_size(op, node_info, data_usage)
            total = ifm // q + ofm // q + start_ret + end_ret
            total_mem_under_q.append(total)

    peak_under_q = max(total_mem_under_q) if total_mem_under_q else 0
    return int(peak_under_q), total_mem_under_q


def estimate_peak_for_given_branch_gsd(
    g_ops,
    schedule,
    q,
    node_info,
    ...
    """
    Same retained-tensor formula as search_dxq(), but q and d/schedule are fixed.

    Original DupNAS branch formula includes:
      - shared input retention
      - finished branch output retention
      - per-op IFM/q + OFM/q + ret_out[bidx]
      - merge/output accumulation point

    This intentionally does NOT add extra conservative boundary retention,
    because search_dxq() does not do that.
    """
    q = max(1, int(q))
    best_d = list(schedule or g_ops)

    branch_info, _ = get_subgraph_branch_info(g_ops, flat_branch_sets, node_info)
    shared_ops, merge_ops, ordered_branches = find_related_shared_merge_ops(
        best_d,
        flat_branch_sets,
        branch_info,
    )

    # If branch metadata cannot be recovered, fallback to the original single-path formula.
    ...
    first_shared_op = shared_ops[0] if shared_ops else None
    shared_in_tid = (
        _safe_first_for_random(node_info[first_shared_op].get("data_in", []))
        if first_shared_op is not None else None
    )
    share_input_size = _tensor_size_for_random(shared_in_tid, data_usage)
    ...
    for br in ordered_branches:
        if not br:
            continue
        last_op = br[-1]
        out_tid = _safe_first_for_random(node_info[last_op].get("data_out", []))
        br_output_size.append(_tensor_size_for_random(out_tid, data_usage))
    ...
    for a in range(q):
        ...
        shared_out_tid = (
            _safe_first_for_random(node_info[first_shared_op].get("data_out", []))
            if first_shared_op is not None else None
        )
        shared_out_size = _tensor_size_for_random(shared_out_tid, data_usage)

        shared_op_io_a = (share_input_size // q) + (shared_out_size // q)
        total_mem_under_q.append(shared_op_ret_a + shared_op_io_a)

        # Same per-op branch calculation as search_dxq().
        for bidx, br in enumerate(ordered_branches):
            for op in br:
                ifm = _op_input_size(op, node_info, data_usage)
                ofm = _op_output_size(op, node_info, data_usage)
        ...
        # Same output accumulation formula as search_dxq().
        sum_out = (base_sum // q) * (a + 1) + (share_input_size // q) * (q - a - 1)
        total_mem_under_q.append(sum_out)
...
def _estimate_model_peak_after_sample(sampled_combos, skipped_bottlenecks, mem_usage, node_info, data_usage, total_sets_of_branches):
    """
    Model-level peak after sampled TS, using original DupNAS-style formulas
    for every sampled (g, s, d/q).

    Model-level peak is:
        max(
            highest original memory among non-selected ops,
            highest evaluated g peak among selected sampled configs,
            skipped/unoptimized bottleneck peak
        )
    """
    selected_ops = set()
    selected_g_peaks = []

    for sel in sampled_combos or []:
        g_ops = list(sel.get('g_ops', []))
        selected_ops.update(g_ops)

        g_peak, total_mem_under_q = estimate_peak_for_given_gsd(
            g_ops=g_ops,
            schedule=sel.get('schedule', g_ops),
            q=sel.get('q', 1),
            node_info=node_info,
            data_usage=data_usage,
            mem_usage=mem_usage,
            total_sets_of_branches=total_sets_of_branches,
        )

        sel['estimated_g_peak_after_ts'] = int(g_peak)
        sel['total_mem_under_q_for_sampled_gsd'] = total_mem_under_q
        selected_g_peaks.append(int(g_peak))

    est_total = list(mem_usage.get('total', []))

    non_selected_peak = 0
    non_selected_peak_idx = -1
    for op, mem in enumerate(est_total):
        if op in selected_ops:
            continue
        if int(mem) > non_selected_peak:
            non_selected_peak = int(mem)
            non_selected_peak_idx = op

    selected_peak = max(selected_g_peaks) if selected_g_peaks else 0
    selected_peak_idx = -1
    if selected_peak and sampled_combos:
        for sel in sampled_combos:
            if int(sel.get('estimated_g_peak_after_ts', 0)) == selected_peak:
                selected_peak_idx = sel.get('bottleneck_op', -1)
                break

    skipped_peak = 0
    skipped_peak_idx = -1
    for sk in skipped_bottlenecks or []:
        op = sk.get('bottleneck_op', -1)
        mem = sk.get('memory_bytes')
        if mem is None and 0 <= op < len(mem_usage.get('total', [])):
            mem = mem_usage['total'][op]
        if mem is None:
            continue
        if int(mem) > skipped_peak:
            skipped_peak = int(mem)
            skipped_peak_idx = op

    peak = max(non_selected_peak, selected_peak, skipped_peak)
    if peak == selected_peak:
        peak_idx = selected_peak_idx
    elif peak == skipped_peak:
        peak_idx = skipped_peak_idx
    else:
        peak_idx = non_selected_peak_idx

    # Compatibility/debug: write each selected g peak to its bottleneck slot.
    for sel in sampled_combos or []:
        bn = sel.get('bottleneck_op', -1)
        if 0 <= bn < len(est_total):
            est_total[bn] = int(sel.get('estimated_g_peak_after_ts', est_total[bn]))

    return peak, peak_idx, est_total, skipped_peak, skipped_peak_idx


def _estimate_model_proxy_after_sample(sampled_combos, node_info):
    """
    DupNAS-style proxy estimate for a sampled model-level combination.

    proxy = computation + 2 * weight access

    Original model proxy is counted for all Conv ops once.
    For each selected g, we replace each Conv op inside g with split-version cost:
        split_macs(q) + 2 * weight_access(q)

    Weight access increases with d/q because spatial pieces reload weights.
    """
    selected_q_by_op = {}
    for sel in sampled_combos or []:
        q = max(1, int(sel.get('q', 1)))
        for op in sel.get('g_ops', []):
            selected_q_by_op[op] = max(selected_q_by_op.get(op, 1), q)

    total_macs_after = 0
    total_weight_access_after = 0

    for op_idx, node in enumerate(node_info):
        if node.get('op_type') != 'Conv':
            continue

        q = selected_q_by_op.get(op_idx, 1)
        try:
            base_macs, split_macs = compute_conv_macs_from_node(node, q)
            macs = split_macs if q > 1 else base_macs
        except Exception:
            macs = 0

        try:
            access = compute_conv_access_from_node(node, q)
        except Exception:
            access = 0

        total_macs_after += int(macs)
        total_weight_access_after += int(access)

    proxy_after_ts = int(total_macs_after + 2 * total_weight_access_after)
    return proxy_after_ts, int(total_macs_after), int(total_weight_access_after), dict(sorted(selected_q_by_op.items()))


def _make_skipped_network_sample_report(reason, failed_item, selected, skipped, already_covered, used_ops, covered_ops, mem_usage, node_info):
    """Return a normal-shaped sample record for a network that must be skipped."""
    est_total = list(mem_usage.get('total', []))
    original_peak = max((int(x) for x in est_total), default=0)
    original_peak_idx = -1
    if est_total:
        original_peak_idx = max(range(len(est_total)), key=lambda i: int(est_total[i]))

    bn = failed_item.get('bottleneck_op', -1) if failed_item else -1
    op_type = failed_item.get('op_type', None) if failed_item else None
    mem_bytes = failed_item.get('memory_bytes') if failed_item else None
    if mem_bytes is None and 0 <= bn < len(est_total):
        mem_bytes = est_total[bn]
    mem_bytes = int(mem_bytes or 0)
    mem_kb = (mem_bytes / 1024) if mem_bytes else 0.0
    ...
    unresolved = {
        ...
        'memory_kb': mem_kb,
        'reason': reason,
        'removed_g_ranges_by_rule_filter': failed_item.get('removed_g_ranges_by_rule_filter', 0) if failed_item else 0,
    }

    skipped_all = list(skipped or []) + [unresolved]
    skipped_over_vm = [sk for sk in skipped_all if int(sk.get('memory_bytes', 0)) > AVAILABLE_VM]
    proxy_after_ts, macs_after_ts, weight_access_after_ts, proxy_q_by_op = _estimate_model_proxy_after_sample(
        selected, node_info
    )
    ...
    return {
        ...
        'skipped': skipped_all,
        'already_covered': list(already_covered or []),
        'unresolved_bottlenecks': [unresolved],
        'skipped_network': True,
        'skip_network_reason': reason,
        'skipped_over_vm': skipped_over_vm,
        'used_ops': sorted(used_ops),
        'covered_ops': sorted(covered_ops),
        'estimated_peak_after_ts': original_peak,
        'estimated_peak_after_ts_idx': original_peak_idx,
        'proxy_after_ts': proxy_after_ts,
        'macs_after_ts': macs_after_ts,
        'weight_access_after_ts': weight_access_after_ts,
        'proxy_q_by_op': proxy_q_by_op,
        'skipped_unoptimized_peak': max((int(sk.get('memory_bytes', 0)) for sk in skipped_all), default=0),
        'skipped_unoptimized_peak_idx': bn,
        'satisfy_vm': False,
        'all_g_rules_ok': False,
    }


def sample_one_model_combination(report, node_info, data_usage, mem_usage, total_sets_of_branches, rng, exgrule='both'):
    """
    Randomly choose one non-overlapping combo per bottleneck.

    Important:
    - used_ops: ops already physically split, cannot be reused.
    - covered_ops: ops already inside selected g, so their bottleneck memory is already optimized.
    - skipped: only truly unoptimized bottlenecks.
    """
    used_ops = set()
    covered_ops = set()
    selected = []
    ...
    for item in report:
        bn = item['bottleneck_op']

        # Case 1: this bottleneck is already inside a previous selected g.
        # It should NOT be selected again, but also should NOT be counted as skipped.
        if bn in covered_ops:
            already_covered.append({
                'bottleneck_op': bn,
                'op_type': item['op_type'],
                'memory_bytes': item.get('memory_bytes', mem_usage['total'][bn]),
                'memory_kb': item.get('memory_kb', mem_usage['total'][bn] / 1024),
                'reason': 'covered by previous selected g',
            })
            continue

        # Fast path: g_details was already rule-filtered during
        # count_random_search_combinations(). Do NOT call _check_g_selection_rules()
        # again inside every random sample; that graph/branch check is expensive and
        # was the reason each seed became slow.
        #
        # We still keep a cheap safety filter using the stored flags and --exgrule.
        # This is normally redundant because g_details is already filtered during
        # count_random_search_combinations(), but it protects old/intermediate reports.
        candidates = [
            gd for gd in item.get('g_details', [])
            if _g_option_matches_exgrule(gd, exgrule)
        ]

        # If rule filtering removed every g option for this bottleneck, this sampled
        # network is invalid. Mark it explicitly and stop sampling this network.
        if not candidates:
            return _make_skipped_network_sample_report(
                reason=f'no g options after stored --exgrule {exgrule} filtering; sampled network skipped',
                failed_item=item,
                selected=selected,
                skipped=skipped,
        ...
        # Build the legal candidate pool once. This avoids shuffling/rechecking a
        # large list and avoids recomputing q_checked_ops for every candidate.
        legal_candidates = []
        for gd in candidates:
            g_ops = list(gd.get('ops', []) or [])
            split_ops = list(gd.get('q_checked_ops', []) or [])
            if not split_ops:
                split_ops = _q_checked_ops(g_ops, node_info, used_ops=set())
            if not split_ops:
                continue

            # Cannot use the same split op twice.
            if any(op in used_ops for op in split_ops):
                continue

            q_list = list(gd.get('q_candidates', []) or [])
            if not q_list:
                continue

            legal_candidates.append((gd, g_ops, split_ops, q_list))

        chosen = None
        if legal_candidates:
            gd, g_ops, split_ops, q_list = rng.choice(legal_candidates)
            q = rng.choice(q_list)

            # Use the stored rule-check result. Do not recompute graph rules here.
            shared_status = gd.get('shared_branch_rule_status', None)
            if shared_status is None:
                shared_status = 'true' if bool(gd.get('shared_branch_rule_ok')) else 'false'
            shared_status = str(shared_status).lower()
            rule_check = {
                'ok': bool(gd.get('boundary_rule_ok')) and (shared_status != 'false'),
                'boundary_rule_ok': bool(gd.get('boundary_rule_ok')),
                'boundary_rule_msg': gd.get('boundary_rule_msg', 'stored rule check'),
                'shared_branch_rule_ok': (shared_status != 'false'),
                'shared_branch_rule_status': shared_status,
                'shared_branch_rule_msg': gd.get('shared_branch_rule_msg', 'stored rule check'),
            }

            schedule, schedule_mode = _sample_schedule_for_g(
                g_ops, node_info, total_sets_of_branches, rng
            )

            chosen = {
                'bottleneck_op': item['bottleneck_op'],
                'op_type': item['op_type'],
                'g_ops': g_ops,
                'source_g_ops': gd.get('source_g_ops', []),
                'boundary_pair': gd.get('boundary_pair', None),
                'bottleneck_input_size': gd.get('bottleneck_input_size', None),
                'start_in': gd.get('start_in', None),
                'end_out': gd.get('end_out', None),
                'boundary_pair_rule_ok': gd.get('boundary_pair_rule_ok', None),
                'split_ops': split_ops,
                'q': q,
                'q_candidates': q_list,
        ...
        # Case 2: no legal non-overlapping candidate, and this bottleneck is not covered.
        # This is truly unoptimized.
        if chosen is None:
            skipped.append({
                'bottleneck_op': item['bottleneck_op'],
                'op_type': item['op_type'],
                'memory_bytes': item.get('memory_bytes', mem_usage['total'][item['bottleneck_op']]),
                'memory_kb': item.get('memory_kb', mem_usage['total'][item['bottleneck_op']] / 1024),
                'reason': 'no non-overlapping candidate available; bottleneck remains unoptimized',
            })
            continue

        selected.append(chosen)

        # These ops are physically split, so they cannot be reused.
        used_ops.update(chosen['split_ops'])

        # These ops are covered by this selected g, so later bottlenecks inside this g
        # should not be counted as skipped/unoptimized.
        covered_ops.update(chosen['g_ops'])

    peak_after, peak_after_idx, est_total, skipped_peak, skipped_peak_idx = _estimate_model_peak_after_sample(
        selected, skipped, mem_usage, node_info, data_usage, total_sets_of_branches
    )

    all_rules_ok = all(sel['rule_check']['ok'] for sel in selected)
    skipped_over_vm = [sk for sk in skipped if int(sk.get('memory_bytes', 0)) > AVAILABLE_VM]
    ...
    return {
        ...
        'used_ops': sorted(used_ops),
        'covered_ops': sorted(covered_ops),
        'estimated_peak_after_ts': peak_after,
        'estimated_peak_after_ts_idx': peak_after_idx,
        'proxy_after_ts': proxy_after_ts,
        'macs_after_ts': macs_after_ts,
        'weight_access_after_ts': weight_access_after_ts,
        'proxy_q_by_op': proxy_q_by_op,
        'skipped_unoptimized_peak': skipped_peak,
        'skipped_unoptimized_peak_idx': skipped_peak_idx,
        'satisfy_vm': (peak_after <= AVAILABLE_VM) and (len(skipped_over_vm) == 0),
        'all_g_rules_ok': all_rules_ok,
    }


def append_random_sample_report(out_path, report, sample_report, seed):
    total_product = _model_level_combination_product(report)
    with open(out_path, 'a') as f:
        f.write('\n\nModel-level summary\n')
        f.write('=' * 50 + '\n')
        f.write(f'TOTAL model-level combinations product: {total_product}\n')
        f.write(f'Random seed: {seed}\n')
        f.write(
            f"Sampled combo: peak_after_ts={sample_report['estimated_peak_after_ts']}B "
            f"({sample_report['estimated_peak_after_ts'] / 1024:.2f}KB) "
            f"proxy_after_ts={sample_report.get('proxy_after_ts', 0)} "
            f"satisfy_vm={sample_report['satisfy_vm']} rules_ok={sample_report['all_g_rules_ok']} "
            f"selected={len(sample_report['selected'])} skipped={len(sample_report['skipped'])} "
            f"skipped_over_vm={len(sample_report['skipped_over_vm'])} "
            f"skipped_peak={sample_report['skipped_unoptimized_peak']}B\n"
        )
        f.write(f"Estimated peak after sampled TS config: {sample_report['estimated_peak_after_ts']} bytes "
                f"({sample_report['estimated_peak_after_ts'] / 1024:.2f} KB), "
                f"peak_op={sample_report['estimated_peak_after_ts_idx']}\n")
        f.write(f"Proxy after sampled TS config: {sample_report.get('proxy_after_ts', 0)} "
                f"(macs_after_ts={sample_report.get('macs_after_ts', 0)}, "
                f"weight_access_after_ts={sample_report.get('weight_access_after_ts', 0)})\n")
        f.write(f"Satisfy VM: {sample_report['satisfy_vm']}\n")
        f.write(f"Skipped/unoptimized bottleneck peak: {sample_report['skipped_unoptimized_peak']} bytes "
                f"({sample_report['skipped_unoptimized_peak'] / 1024:.2f} KB), "
                f"peak_op={sample_report['skipped_unoptimized_peak_idx']}\n")
        f.write(f"Skipped bottlenecks still over VM: {len(sample_report['skipped_over_vm'])}\n")
        f.write(f"Skipped network: {sample_report.get('skipped_network', False)}\n")
        if sample_report.get('skipped_network', False):
            f.write(f"Skip-network reason: {sample_report.get('skip_network_reason', '')}\n")
            f.write(f"Unresolved bottlenecks: {sample_report.get('unresolved_bottlenecks', [])}\n")
        f.write(f"All selected g rules OK: {sample_report['all_g_rules_ok']}\n")
        f.write(f"Used split ops: {sample_report['used_ops']}\n")

        f.write('\nSelected random model combination:\n')
        for sel in sample_report['selected']:
            f.write(
                f"  bottleneck={sel['bottleneck_op']}({sel['op_type']}), "
                f"g_ops={sel['g_ops']}, split_ops={sel['split_ops']}, q={sel['q']}, "
                f"schedule_mode={sel['schedule_mode']}, schedule={sel['schedule']}\n"
            )
            rc = sel['rule_check']
            f.write(f"    rule_ok={rc['ok']}, boundary={rc['boundary_rule_ok']} ({rc['boundary_rule_msg']}), "
                    f"shared={rc['shared_branch_rule_ok']} status={rc.get('shared_branch_rule_status')} ({rc['shared_branch_rule_msg']})\n")

        if sample_report['skipped']:
            f.write('\nSkipped bottlenecks, kept original memory in peak_after_ts:\n')
            for sk in sample_report['skipped']:
                f.write(
                    f"  bottleneck={sk['bottleneck_op']}({sk['op_type']}), "
                    f"mem={sk['memory_bytes']} bytes ({sk['memory_kb']:.2f} KB): "
                    f"{sk['reason']}\n"
                )


def _sample_rule_counts(sr):
    selected = list(sr.get('selected', []))
    total_g = len(selected)
    ok_g = sum(1 for sel in selected if sel.get('rule_check', {}).get('ok', False))
    missing_g = max(0, total_g - ok_g)
    return ok_g, total_g, missing_g
...
def _format_sample_gds(sr, report):
    """
    Compactly format the chosen (g, d, s) for every bottleneck in one sampled model combo.

    Here:
      g = selected graph/operator set
      d = duplication / split factor q
      s = selected schedule mode and schedule order

    Skipped bottlenecks are still listed so every bottleneck has one entry.
    """
    selected_by_bn = {sel['bottleneck_op']: sel for sel in sr.get('selected', [])}
    skipped_by_bn = {sk['bottleneck_op']: sk for sk in sr.get('skipped', [])}
    parts = []
    if sr.get('skipped_network', False):
        parts.append(f"NETWORK_SKIPPED:reason={sr.get('skip_network_reason', '')}")

    for item in report:
        bn = item.get('bottleneck_op')
        op_type = item.get('op_type', '?')
        if bn in selected_by_bn:
            sel = selected_by_bn[bn]
            rule_ok = sel.get('rule_check', {}).get('ok', False)
            parts.append(
                f"bn{bn}({op_type}):"
                f"g={sel.get('g_ops', [])},"
                f"d={sel.get('q', None)},"
                f"s={sel.get('schedule_mode', None)},"
                f"sched={sel.get('schedule', [])},"
                f"rule_ok={rule_ok}"
            )
        elif bn in skipped_by_bn:
            sk = skipped_by_bn[bn]
            parts.append(
                f"bn{bn}({op_type}):SKIP,"
                f"mem={sk.get('memory_bytes', 0)}B,"
                f"reason={sk.get('reason', '')}"
            )
        else:
            # Safety fallback: should not happen, but keeps the report complete.
            parts.append(f"bn{bn}({op_type}):MISSING")
...
def _best_proxy_sample_with_feasible_priority(sample_reports):
    """
    Pick best proxy sample with priority:
      1) among feasible samples, choose smallest proxy_after_ts
      2) if no feasible sample exists, choose smallest proxy_after_ts among all samples

    Returns:
      (sample_idx, sample_report, selected_from_feasible_pool)
    """
    if not sample_reports:
        return None, None, False

    feasible_pool = [
        (i, sr) for i, sr in enumerate(sample_reports)
        if sr.get('satisfy_vm', False)
    ]

    if feasible_pool:
        best_idx, best_sr = min(feasible_pool, key=lambda x: int(x[1].get('proxy_after_ts', 0)))
        return best_idx, best_sr, True

    best_idx, best_sr = min(
        enumerate(sample_reports),
        key=lambda x: int(x[1].get('proxy_after_ts', 0))
    )
    return best_idx, best_sr, False


def _format_selected_gsd_details(sr):
    """
    Format all selected g/s/d configs in one sample.
    d = q, s = schedule mode/order.
    """
    if sr is None:
        return 'None'

    parts = []
    for sel in sr.get('selected', []):
        rc = sel.get('rule_check', {})
        parts.append(
            f"bn={sel.get('bottleneck_op')}({sel.get('op_type')}), "
            f"g={sel.get('g_ops', [])}, "
            f"d/q={sel.get('q')}, "
            f"s={sel.get('schedule_mode')}, "
            f"schedule={sel.get('schedule', [])}, "
            f"g_peak={sel.get('estimated_g_peak_after_ts', None)}, "
            f"rule_ok={rc.get('ok')}, "
            f"boundary_rule_ok={rc.get('boundary_rule_ok')}, "
            f"shared_branch_rule_ok={rc.get('shared_branch_rule_ok')}, shared_branch_rule_status={rc.get('shared_branch_rule_status')}"
        )

    return ' | '.join(parts) if parts else 'None'


def append_random_samples_report(out_path, report, sample_reports, seed, total_sampling_sec=None):
    """
    Write compact records for many sampled model-level TS combinations.
    Each record keeps the fields needed to compare random combinations.
    """
    total_product = _model_level_combination_product(report)
    ...
    rules_ok_count = 0
    partial_rule_counts = defaultdict(int)
    feasible_partial_rule_counts = defaultdict(int)

    for i, sr in enumerate(sample_reports):
        peak = int(sr['estimated_peak_after_ts'])
        ...
        if sr['satisfy_vm']:
            feasible_count += 1
        if sr['all_g_rules_ok']:
            rules_ok_count += 1
        ok_g, total_g, missing_g = _sample_rule_counts(sr)
        # Count partial Rule-OK samples regardless of VM feasibility.
        # Example: total_g=5 and ok_g=4 => missing_g=1 => "-1 Rule-OK".
        if missing_g > 0:
            partial_rule_counts[missing_g] += 1
        # Also keep the stricter bucket: partial Rule-OK AND satisfy VM.
        if sr['satisfy_vm'] and missing_g > 0:
            feasible_partial_rule_counts[missing_g] += 1

    with open(out_path, 'w') as f:
        f.write('\n\nRandom sampled model combinations\n')
        f.write('=' * 50 + '\n')
        f.write(f'TOTAL model-level combinations product: {total_product}\n')
        f.write(f'Random seed: {seed}\n')
        f.write(f'Random samples: {len(sample_reports)}\n')
        if total_sampling_sec is not None:
            avg_sampling_sec = total_sampling_sec / max(1, len(sample_reports))
            f.write(f'Total sampled-combo time: {total_sampling_sec:.6f} sec\n')
            f.write(f'Average time per sampled combo: {avg_sampling_sec:.6f} sec\n')
        feasible_and_rules_ok_count = sum(
            1 for sr in sample_reports
            if sr['satisfy_vm'] and sr['all_g_rules_ok']
        )
        f.write(f'Feasible samples satisfy VM: {feasible_count}/{len(sample_reports)}\n')
        f.write(f'Skipped networks due to unresolved/no-g bottleneck: {skipped_network_count}/{len(sample_reports)}\n')
        f.write(f'Rule-OK samples: {rules_ok_count}/{len(sample_reports)}\n')
        f.write(f'Feasible AND Rule-OK samples: {feasible_and_rules_ok_count}/{len(sample_reports)}\n')
        max_selected_g = max((len(sr.get('selected', [])) for sr in sample_reports), default=0)

        f.write('Partial Rule-OK samples by missing-rule count, regardless of VM feasibility:\n')
        for missing_g in range(1, max_selected_g + 1):
            f.write(f'  -{missing_g} Rule-OK samples: {partial_rule_counts.get(missing_g, 0)}/{len(sample_reports)}\n')

        f.write('Partial Rule-OK AND feasible samples satisfy VM by missing-rule count:\n')
        for missing_g in range(1, max_selected_g + 1):
            f.write(f'  -{missing_g} Rule-OK and feasible samples: {feasible_partial_rule_counts.get(missing_g, 0)}/{len(sample_reports)}\n')
        if best_idx is not None:
            f.write(f'Best sampled peak: sample_id={best_idx}, peak_after_ts={best_peak}B ({best_peak / 1024:.2f}KB)\n')
        best_proxy_idx, best_proxy_sr, best_proxy_from_feasible = _best_proxy_sample_with_feasible_priority(sample_reports)
        if best_proxy_idx is not None:
            best_proxy = int(best_proxy_sr.get('proxy_after_ts', 0))
            f.write(
                f'Best sampled proxy under feasible-priority: sample_id={best_proxy_idx}, '
                f'proxy_after_ts={best_proxy}, feasible={best_proxy_sr.get("satisfy_vm", False)}, '
                f'selected_from_feasible_pool={best_proxy_from_feasible}\n'
            )
            if not best_proxy_sr.get('satisfy_vm', False):
                f.write('  NOTE: No feasible sample was found; this is the smallest proxy among infeasible samples.\n')
            f.write(
                f'  peak_after_ts={best_proxy_sr.get("estimated_peak_after_ts", 0)}B '
                f'({best_proxy_sr.get("estimated_peak_after_ts", 0) / 1024:.2f}KB)\n'
            )
            f.write(f'  g/s/d: {_format_selected_gsd_details(best_proxy_sr)}\n')

        f.write('\n')
        f.write('sample_id,time_sec,peak_after_ts_B,peak_after_ts_KB,proxy_after_ts,macs_after_ts,weight_access_after_ts,satisfy_vm,rules_ok,selected,rule_ok_g,rule_total_g,rule_missing_g,skipped,skipped_over_vm,skipped_peak_B,skipped_network,unresolved_no_g_count,skip_network_reason,chosen_gds\n')
        for i, sr in enumerate(sample_reports):
            ok_g, total_g, missing_g = _sample_rule_counts(sr)
            f.write(
                f"{i},"
                f"{sr.get('sample_time_sec', 0.0):.6f},"
                f"{sr['estimated_peak_after_ts']},"
                f"{sr['estimated_peak_after_ts'] / 1024:.2f},"
                f"{sr.get('proxy_after_ts', 0)},"
                f"{sr.get('macs_after_ts', 0)},"
                f"{sr.get('weight_access_after_ts', 0)},"
                f"{sr['satisfy_vm']},"
                f"{sr['all_g_rules_ok']},"
                f"{len(sr['selected'])},"
                f"{ok_g},"
                ...
                f"{missing_g},"
                f"{len(sr['skipped'])},"
                f"{len(sr['skipped_over_vm'])},"
                f"{sr['skipped_unoptimized_peak']},"
                f"{sr.get('skipped_network', False)},"
                f"{len(sr.get('unresolved_bottlenecks', []))},"
                f"{_csv_escape_cell(sr.get('skip_network_reason', ''))},"
                f"{_csv_escape_cell(_format_sample_gds(sr, report))}\n"
            )

        f.write('\nPer-selected-g sample records:\n')
        f.write(
            'sample_id,bottleneck_op,op_type,peak_after_ts_B,peak_after_ts_KB,'
            'proxy_after_ts,satisfy_vm,sample_rules_ok,'
            'g_rule_ok,boundary_rule_ok,shared_branch_rule_ok,shared_branch_rule_status,'
            'd_q,s_mode,schedule,g_ops,source_g_ops,boundary_pair,'
            'bottleneck_input,start_in,end_out,estimated_g_peak_after_ts\n'
        )
        for i, sr in enumerate(sample_reports):
            for sel in sr.get('selected', []):
                rc = sel.get('rule_check', {})
                f.write(
                    f"{i},"
                    f"{sel.get('bottleneck_op')},"
                    f"{sel.get('op_type')},"
                    f"{sr.get('estimated_peak_after_ts', 0)},"
                    f"{sr.get('estimated_peak_after_ts', 0) / 1024:.2f},"
                    f"{sr.get('proxy_after_ts', 0)},"
                    f"{sr.get('satisfy_vm')},"
                    f"{sr.get('all_g_rules_ok')},"
                    f"{rc.get('ok')},"
                    f"{rc.get('boundary_rule_ok')},"
                    f"{rc.get('shared_branch_rule_ok')},"
                    f"{rc.get('shared_branch_rule_status')},"
                    f"{sel.get('q')},"
                    f"{sel.get('schedule_mode')},"
                    f"{_csv_escape_cell(sel.get('schedule', []))},"
                    f"{_csv_escape_cell(sel.get('g_ops', []))},"
                    f"{_csv_escape_cell(sel.get('source_g_ops', []))},"
                    f"{_csv_escape_cell(sel.get('boundary_pair', None))},"
                    f"{sel.get('bottleneck_input_size', None)},"
                    f"{sel.get('start_in', None)},"
                    f"{sel.get('end_out', None)},"
                    f"{sel.get('estimated_g_peak_after_ts', None)}\n"
                )
        ...
        for i, sr in enumerate(sample_reports):
            ok_g, total_g, missing_g = _sample_rule_counts(sr)
            f.write(
                f"Sampled combo {i}: time={sr.get('sample_time_sec', 0.0):.6f}s "
                f"peak_after_ts={sr['estimated_peak_after_ts']}B "
                f"({sr['estimated_peak_after_ts'] / 1024:.2f}KB) "
                f"proxy_after_ts={sr.get('proxy_after_ts', 0)} "
                f"satisfy_vm={sr['satisfy_vm']} rules_ok={sr['all_g_rules_ok']} "
                f"skipped_network={sr.get('skipped_network', False)} "
                f"selected={len(sr['selected'])} rule_ok_g={ok_g}/{total_g} rule_missing_g={missing_g} "
                f"skipped={len(sr['skipped'])} skipped_over_vm={len(sr['skipped_over_vm'])} "
                f"unresolved_no_g={len(sr.get('unresolved_bottlenecks', []))} "
                f"skip_reason={sr.get('skip_network_reason', '')} "
                f"skipped_peak={sr['skipped_unoptimized_peak']}B "
                f"chosen_gds={_format_sample_gds(sr, report)}\n"
            )


def _summarize_sample_reports(sample_reports):
    """Return compact statistics for a list of sampled model-level combinations."""
    feasible_count = 0
    skipped_network_count = 0
    rules_ok_count = 0
    feasible_and_rules_ok_count = 0
    partial_rule_counts = defaultdict(int)
    feasible_partial_rule_counts = defaultdict(int)
    best_idx = None
    best_peak = None
    ...
    best_proxy_idx, best_proxy_sr, best_proxy_from_feasible = _best_proxy_sample_with_feasible_priority(sample_reports)
    best_proxy = int(best_proxy_sr.get('proxy_after_ts', 0)) if best_proxy_sr is not None else 0
    best_proxy_feasible = bool(best_proxy_sr.get('satisfy_vm', False)) if best_proxy_sr is not None else False
    ...
    for i, sr in enumerate(sample_reports):
        ...
        if sr['all_g_rules_ok']:
            rules_ok_count += 1
        if sr['satisfy_vm'] and sr['all_g_rules_ok']:
            feasible_and_rules_ok_count += 1

        ok_g, total_g, missing_g = _sample_rule_counts(sr)
        max_selected_g = max(max_selected_g, total_g)
    ...
    return {
        ...
        'skipped_network_count': skipped_network_count,
        'rules_ok_count': rules_ok_count,
        'feasible_and_rules_ok_count': feasible_and_rules_ok_count,
        'partial_rule_counts': partial_rule_counts,
        'feasible_partial_rule_counts': feasible_partial_rule_counts,
        'best_idx': best_idx,
        'best_peak': best_peak if best_peak is not None else 0,
        'best_proxy_idx': best_proxy_idx,
        'best_proxy': best_proxy,
        'best_proxy_feasible': best_proxy_feasible,
        'best_proxy_from_feasible_pool': best_proxy_from_feasible,
        'max_selected_g': max_selected_g,
    }

def _parse_random_seeds(args):
    """Support either --random_seed 0 or --random_seeds 0,1,2 / \"0 1 2\"."""
    if getattr(args, 'random_seeds', None):
        raw = args.random_seeds.replace(',', ' ').split()
        seeds = []
        for x in raw:
            if x.strip() == '':
                continue
            seeds.append(int(x))
        if not seeds:
            raise ValueError('--random_seeds was provided but no valid integer seeds were found')
        return seeds
    return [args.random_seed]


def export_all_seed_summary_report(out_path, combo_report, seed_results, random_sample_num, total_all_seed_time):
    """
    Write one compact ALLSEEDS report that aggregates all seeds for one model/VM run.

    This compact version does NOT write:
      - combined all-seed sample records
      - combined all-seed per-selected-g records
      - readable sampled combo lines

    It keeps the fields needed for collection:
      - TOTAL SAMPLES
      - TOTAL TIME
      - AVG TIME
      - Overall feasible samples satisfy VM
      - Overall best sampled peak, including peak_after_ts and its g/s/d
      - Overall best sampled proxy summary
      - PER-SEED SUMMARY, including each seed best sampled peak g/s/d
    """
    total_samples = sum(len(x['sample_reports']) for x in seed_results)
    avg_time = total_all_seed_time / max(1, total_samples)
    total_product = _model_level_combination_product(combo_report)

    overall_feasible = 0
    overall_skipped_network = 0
    overall_rules_ok = 0
    overall_feasible_and_rules_ok = 0
    overall_partial = defaultdict(int)
    overall_feasible_partial = defaultdict(int)
    overall_best = None
    overall_best_proxy = None
    max_selected_g_overall = 0

    seed_summaries = []
    for entry in seed_results:
        summary = _summarize_sample_reports(entry['sample_reports'])
        summary['seed'] = entry['seed']
        summary['total_sampling_sec'] = entry['total_sampling_sec']
        summary['avg_sampling_sec'] = entry['total_sampling_sec'] / max(1, summary['num_samples'])
        summary['random_output_file'] = entry.get('random_output_file', 'NOT_WRITTEN_TO_SAVE_FILE_SIZE')
        seed_summaries.append(summary)

        overall_feasible += summary['feasible_count']
        overall_skipped_network += summary.get('skipped_network_count', 0)
        overall_rules_ok += summary['rules_ok_count']
        overall_feasible_and_rules_ok += summary.get('feasible_and_rules_ok_count', 0)
        max_selected_g_overall = max(max_selected_g_overall, summary['max_selected_g'])

        for k, v in summary['partial_rule_counts'].items():
            overall_partial[k] += v
        for k, v in summary['feasible_partial_rule_counts'].items():
            overall_feasible_partial[k] += v

        if summary['best_idx'] is not None:
            candidate = (summary['best_peak'], entry['seed'], summary['best_idx'])
            if overall_best is None or candidate[0] < overall_best[0]:
                overall_best = candidate

        if summary.get('best_proxy_idx') is not None:
            # Feasible-priority across all seeds:
            # feasible samples beat infeasible samples; then minimize proxy.
            proxy_candidate = (
                0 if summary.get('best_proxy_feasible', False) else 1,
                summary.get('best_proxy', 0),
                entry['seed'],
                summary.get('best_proxy_idx'),
                summary.get('best_proxy_feasible', False),
            )
            if overall_best_proxy is None or proxy_candidate < overall_best_proxy:
                overall_best_proxy = proxy_candidate

    with open(out_path, 'w') as f:
        f.write('ALL-SEED RANDOM SAMPLE SUMMARY REPORT\n')
        f.write('=' * 50 + '\n')
        f.write(f'TOTAL model-level combinations product: {total_product}\n')
        f.write(f'Bottlenecks with 0 valid g options: {_zero_valid_g_bottleneck_count(combo_report)}\n')
        f.write('Seeds: ' + ', '.join(str(x['seed']) for x in seed_results) + '\n')
        f.write(f'Random samples per seed: {random_sample_num}\n')
        f.write(f'TOTAL SAMPLES: {total_samples}\n')
        f.write(f'TOTAL TIME: {total_all_seed_time:.6f} sec\n')
        f.write(f'AVG TIME: {avg_time:.6f} sec/sample\n')
        f.write(f'Overall feasible samples satisfy VM: {overall_feasible}/{total_samples}\n')
        f.write(f'Overall skipped networks due to unresolved/no-g bottleneck: {overall_skipped_network}/{total_samples}\n')
        f.write(f'Overall Rule-OK samples: {overall_rules_ok}/{total_samples}\n')
        f.write(f'Overall Feasible AND Rule-OK samples: {overall_feasible_and_rules_ok}/{total_samples}\n')

        f.write('Overall Partial Rule-OK samples by missing-rule count, regardless of VM feasibility:\n')
        for missing_g in range(1, max_selected_g_overall + 1):
            f.write(f'  -{missing_g} Rule-OK samples: {overall_partial.get(missing_g, 0)}/{total_samples}\n')

        f.write('Overall Partial Rule-OK AND feasible samples satisfy VM by missing-rule count:\n')
        for missing_g in range(1, max_selected_g_overall + 1):
            f.write(f'  -{missing_g} Rule-OK and feasible samples: {overall_feasible_partial.get(missing_g, 0)}/{total_samples}\n')

        if overall_best is not None:
            best_peak, best_seed, best_idx = overall_best
            best_peak_sr = None
            for entry in seed_results:
                if entry['seed'] == best_seed and 0 <= best_idx < len(entry['sample_reports']):
                    best_peak_sr = entry['sample_reports'][best_idx]
                    break

            f.write(
                f'Overall best sampled peak: seed={best_seed}, sample_id={best_idx}, '
                f'peak_after_ts={best_peak}B ({best_peak / 1024:.2f}KB)\n'
            )
            if best_peak_sr is not None:
                f.write(f'  g/s/d: {_format_selected_gsd_details(best_peak_sr)}\n')

        if overall_best_proxy is not None:
            _, best_proxy, best_proxy_seed, best_proxy_idx, best_proxy_feasible = overall_best_proxy
            best_proxy_sr = None
            for entry in seed_results:
                if entry['seed'] == best_proxy_seed and 0 <= best_proxy_idx < len(entry['sample_reports']):
                    best_proxy_sr = entry['sample_reports'][best_proxy_idx]
                    break
            f.write(
                f'Overall best sampled proxy under feasible-priority: seed={best_proxy_seed}, '
                f'sample_id={best_proxy_idx}, proxy_after_ts={best_proxy}, feasible={best_proxy_feasible}\n'
            )
            if best_proxy_sr is not None:
                if not best_proxy_sr.get('satisfy_vm', False):
                    f.write('  NOTE: No feasible sample was found across all seeds; this is the smallest proxy among infeasible samples.\n')
                f.write(
                    f'  peak_after_ts={best_proxy_sr.get("estimated_peak_after_ts", 0)}B '
                    f'({best_proxy_sr.get("estimated_peak_after_ts", 0) / 1024:.2f}KB)\n'
                )
        ...
        for summary in seed_summaries:
            ...
            f.write(f'\nSeed {seed}\n')
            f.write('-' * 50 + '\n')
            f.write(f'Report file: {summary["random_output_file"]}\n')
            f.write(f'Random samples: {n}\n')
            f.write(f'Total sampled-combo time: {summary["total_sampling_sec"]:.6f} sec\n')
            f.write(f'Average time per sampled combo: {summary["avg_sampling_sec"]:.6f} sec\n')
            f.write(f'Feasible samples satisfy VM: {summary["feasible_count"]}/{n}\n')
            f.write(f'Skipped networks due to unresolved/no-g bottleneck: {summary.get("skipped_network_count", 0)}/{n}\n')
            f.write(f'Rule-OK samples: {summary["rules_ok_count"]}/{n}\n')
            f.write(f'Feasible AND Rule-OK samples: {summary.get("feasible_and_rules_ok_count", 0)}/{n}\n')
            f.write('Partial Rule-OK samples by missing-rule count, regardless of VM feasibility:\n')
            for missing_g in range(1, summary['max_selected_g'] + 1):
                f.write(f'  -{missing_g} Rule-OK samples: {summary["partial_rule_counts"].get(missing_g, 0)}/{n}\n')
            f.write('Partial Rule-OK AND feasible samples satisfy VM by missing-rule count:\n')
            for missing_g in range(1, summary['max_selected_g'] + 1):
                f.write(f'  -{missing_g} Rule-OK and feasible samples: {summary["feasible_partial_rule_counts"].get(missing_g, 0)}/{n}\n')

            if summary['best_idx'] is not None:
                best_peak_idx = summary['best_idx']
                best_peak_sr = None
                for entry in seed_results:
                    if entry['seed'] == seed and 0 <= best_peak_idx < len(entry['sample_reports']):
                        best_peak_sr = entry['sample_reports'][best_peak_idx]
                        break
                f.write(f'Best sampled peak: sample_id={best_peak_idx}, peak_after_ts={summary["best_peak"]}B ({summary["best_peak"] / 1024:.2f}KB)\n')
                if best_peak_sr is not None:
                    f.write(f'  g/s/d: {_format_selected_gsd_details(best_peak_sr)}\n')

            if summary.get('best_proxy_idx') is not None:
                best_proxy_idx = summary["best_proxy_idx"]
                best_proxy_sr = None
                for entry in seed_results:
                    if entry['seed'] == seed and 0 <= best_proxy_idx < len(entry['sample_reports']):
                        best_proxy_sr = entry['sample_reports'][best_proxy_idx]
                        break
                f.write(
                    f'Best sampled proxy under feasible-priority: sample_id={best_proxy_idx}, '
                    f'proxy_after_ts={summary["best_proxy"]}, '
                    f'feasible={summary.get("best_proxy_feasible", False)}, '
                    f'selected_from_feasible_pool={summary.get("best_proxy_from_feasible_pool", False)}\n'
                )
                if best_proxy_sr is not None:
                    if not best_proxy_sr.get('satisfy_vm', False):
                        f.write('  NOTE: No feasible sample was found for this seed; this is the smallest proxy among infeasible samples.\n')
                    f.write(
                        f'  peak_after_ts={best_proxy_sr.get("estimated_peak_after_ts", 0)}B '
                        f'({best_proxy_sr.get("estimated_peak_after_ts", 0) / 1024:.2f}KB)\n'
                    )
...
def export_combination_count_report(report, out_path):
    with open(out_path, 'w') as f:
        f.write('Random-search combination count report\n')
        f.write('=' * 50 + '\n')
        f.write(f"TOTAL model-level combinations product: {_model_level_combination_product(report)}\n")
        f.write(f"Bottlenecks with 0 valid g options: {_zero_valid_g_bottleneck_count(report)}\n")
        if report:
            f.write(f"Excluded-rule option mode (--exgrule): {report[0].get('exgrule', 'both')}\n")
        for item in report:
            f.write(f"\nBottleneck Op {item['bottleneck_op']} ({item['op_type']})\n")
            f.write(f"  Memory: {item['memory_bytes']} bytes ({item['memory_kb']:.2f} KB)\n")
            f.write(f"  --exgrule mode: {item.get('exgrule', 'both')}\n")
            f.write(f"  #pair-expanded g ranges after rule filter: {item['total_g_ranges']}\n")
            f.write(f"  #g ranges removed by rule filter: {item.get('removed_g_ranges_by_rule_filter', 0)}\n")
            if item.get('has_no_g_options_after_filter', False):
                f.write("  WARNING: no g options remain after rule filtering; sampled networks that reach this bottleneck will be skipped.\n")
            f.write(f"  q union: {item['duplication_factors_union']}\n")
            f.write(f"  schedule modes: {item['schedule_modes']}\n")
            f.write(f"  TOTAL combinations: {item['total_combinations']}\n")
            if item.get('removed_g_details_by_rule_filter'):
                f.write('  Removed g examples by rule filter (first 20):\n')
                for rd in item.get('removed_g_details_by_rule_filter', []):
                    f.write(
                        f"    g={rd.get('ops')}, pair={rd.get('boundary_pair')}, "
                        f"source_g={rd.get('source_g_ops')}, "
                        f"bottleneck_input={rd.get('bottleneck_input_size')}, "
                        f"start_in={rd.get('start_in')}, end_out={rd.get('end_out')}, "
                        f"reason={rd.get('reason')}\n"
                    )
            f.write('  Per-g details:\n')
            for gd in item['g_details']:
                f.write(
                    f"    g={gd['g']}, "
                    f"pair={gd.get('boundary_pair')}, "
                    f"source_g={gd.get('source_g_ops')}, "
                    f"bottleneck_input={gd.get('bottleneck_input_size')}, "
                    f"start_in={gd.get('start_in')}, end_out={gd.get('end_out')}, "
                    f"q={gd['q_candidates']}, "
                    f"schedule={gd['schedule_mode']}({gd['schedule_num']}), "
                    f"comb={gd['combination_num']}\n"
                )
                if gd['schedule_samples']:
                    f.write(f"      schedule_samples={gd['schedule_samples']}\n")
...
def model_tracing(onnx_path, onnx_name):
    ...
    os.makedirs(os.path.join(onnx_path, IFD_DIR), exist_ok=True)
    ...
    if ENABLE_COUNT_MODE:
        combo_report = count_random_search_combinations(
            node_info=node_info,
            data_usage=data_usage,
            mem_usage=mem_usage,
            total_sets_of_branches=total_sets_of_branches,
            used_ops=set(),
            max_materialize_schedules=200,
            exgrule=args.exgrule,
        )
        exgrule_suffix = "_" + str(args.exgrule)
        output_file = onnx_path + OUTDIR + onnx_name + "_combo_count_VM" + str(AVAILABLE_VM // 1024) + exgrule_suffix + ".txt"
        export_combination_count_report(combo_report, output_file)

        random_sample_num = max(1, int(args.random_samples))
        seed_list = _parse_random_seeds(args)
        seed_results = []
        all_seed_start_time = time.perf_counter()

        for seed in seed_list:
            random_output_file = "NOT_WRITTEN_TO_SAVE_FILE_SIZE"
            rng = random.Random(seed)
            sample_reports = []
            sampling_start_time = time.perf_counter()
            for _sample_i in range(random_sample_num):
                combo_start_time = time.perf_counter()
                sr = sample_one_model_combination(
                    combo_report, node_info, data_usage, mem_usage, total_sets_of_branches, rng,
                    exgrule=args.exgrule
                )
                sr['sample_time_sec'] = time.perf_counter() - combo_start_time
                sample_reports.append(sr)
            total_sampling_sec = time.perf_counter() - sampling_start_time
            # append_random_samples_report(random_output_file, combo_report, sample_reports, seed, total_sampling_sec)
            seed_results.append({
                'seed': seed,
                'sample_reports': sample_reports,
                'total_sampling_sec': total_sampling_sec,
                'random_output_file': random_output_file,
            })

        total_all_seed_time = time.perf_counter() - all_seed_start_time
        all_seed_summary_file = (
            onnx_path + OUTDIR + onnx_name + "_combo_random_samples_ALLSEEDS_VM" +
            str(AVAILABLE_VM // 1024) +  exgrule_suffix + ".txt"
        )
        export_all_seed_summary_report(all_seed_summary_file, combo_report, seed_results, random_sample_num, total_all_seed_time)

        # Keep these variables for the existing console print below; use the last seed.
        sample_reports = seed_results[-1]['sample_reports']
        total_sampling_sec = seed_results[-1]['total_sampling_sec']
        random_output_file = seed_results[-1]['random_output_file']
        best_sample_idx = min(
            range(len(sample_reports)),
            key=lambda i: sample_reports[i]['estimated_peak_after_ts']
        )
        best_sample = sample_reports[best_sample_idx]
        feasible_count = sum(1 for sr in sample_reports if sr['satisfy_vm'])
        rules_ok_count = sum(1 for sr in sample_reports if sr['all_g_rules_ok'])
        feasible_and_rules_ok_count = sum(
            1 for sr in sample_reports
            if sr['satisfy_vm'] and sr['all_g_rules_ok']
        )
        ...
        print(f"[COUNT] Bottlenecks over VM: {which_over_mem}")
        print(f"[COUNT] TOTAL model-level combinations product: {_model_level_combination_product(combo_report)}")
        print(f"[COUNT] Bottlenecks with 0 valid g options: {_zero_valid_g_bottleneck_count(combo_report)}")
        for item in combo_report:
            print(
                f"[COUNT] op={item['bottleneck_op']} type={item['op_type']} "
                f"mem={item['memory_bytes']}B g={item['total_g_ranges']} "
                f"q_union={item['duplication_factors_union']} "
                f"total_comb={item['total_combinations']}"
            )
        print(f"[COUNT] Random sampled combos: {len(sample_reports)}")
        print(f"[COUNT] Sampling time: total={total_sampling_sec:.6f}s avg={total_sampling_sec / max(1, len(sample_reports)):.6f}s per combo")
        print(f"[COUNT] Samples satisfying VM: {feasible_count}/{len(sample_reports)}")
        print(f"[COUNT] Samples rules OK: {rules_ok_count}/{len(sample_reports)}")
        print(f"[COUNT] Samples satisfying VM and rules OK: {feasible_and_rules_ok_count}/{len(sample_reports)}")
        max_selected_g = max((len(sr.get('selected', [])) for sr in sample_reports), default=0)
        for missing_g in range(1, max_selected_g + 1):
            print(f"[COUNT] Feasible samples with -{missing_g} Rule-OK: {feasible_partial_rule_counts.get(missing_g, 0)}/{len(sample_reports)}")
        print(
            f"[COUNT] Best sampled combo {best_sample_idx}: peak_after_ts={best_sample['estimated_peak_after_ts']}B "
            f"({best_sample['estimated_peak_after_ts'] / 1024:.2f}KB) "
            f"satisfy_vm={best_sample['satisfy_vm']} rules_ok={best_sample['all_g_rules_ok']} "
            f"selected={len(best_sample['selected'])} skipped={len(best_sample['skipped'])} "
            f"skipped_over_vm={len(best_sample['skipped_over_vm'])} "
            f"skipped_peak={best_sample['skipped_unoptimized_peak']}B"
        )
        best_proxy_idx_console, best_proxy_sr_console, best_proxy_from_feasible_console = _best_proxy_sample_with_feasible_priority(sample_reports)
        if best_proxy_idx_console is not None:
            print(
                f"[COUNT] Best proxy under feasible-priority {best_proxy_idx_console}: "
                f"proxy_after_ts={best_proxy_sr_console.get('proxy_after_ts', 0)} "
                f"satisfy_vm={best_proxy_sr_console.get('satisfy_vm', False)} "
                f"selected_from_feasible_pool={best_proxy_from_feasible_console} "
                f"g/s/d={_format_selected_gsd_details(best_proxy_sr_console)}"
            )
        print(f"[COUNT] Saved count report: {output_file}")
        print("[COUNT] Per-seed random-sample reports are not written.")
        print(f"[COUNT] Saved compact all-seed summary report: {all_seed_summary_file}")
        return which_over_mem, mem_usage['peak'], combo_report
...
def run_test(onnx_file_path):
    ...
    if ENABLE_COUNT_MODE:
        which_over_mem, oripeak, combo_report = model_tracing(onnx_path, model_name)
    elif ENABLE_PDQSEL_MODE:
    ...
    if ENABLE_COUNT_MODE:
        return full_duration, combo_report
    elif ENABLE_PDQSEL_MODE:
...
if __name__ == "__main__":
    ...
    if ENABLE_COUNT_MODE:
        full_sec, combo_report = run_test(test_model_path)
    elif ENABLE_PDQSEL_MODE:
