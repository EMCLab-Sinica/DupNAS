...
def initialize(global_settings: Settings):
    # [MODIFIED] keep initialize simple; no global dist init
    utils.set_seed(global_settings.NAS_SETTINGS_GENERAL['SEED'])
    os.environ["CUDA_DEVICE_ORDER"] = "PCI_BUS_ID"
    
    # Remote logger only on rank-0 so multi-proc won’t duplicate runs
    if global_settings.GLOBAL_SETTINGS['USE_REMOTE_LOGGER'] and is_rank0_env():  # [MODIFIED]
...
def run_nas(global_settings: Settings):
    ...
    # -- STAGE 1: Search Space Optimization (rank-0 only)  [MODIFIED]
    if Stages.SS_OPT in stages and is_rank0_env():  # [MODIFIED]
        stage_ss_optimization(global_settings, dataset, supernet_name, stage_ss_opt_logfname)
        stages_completed.append(Stages.SS_OPT)

    # -- STAGE 2: Train Supernet (one-shot)  [MODIFIED]
    # Only Stage-2 runs on ALL ranks if --dist!=none; otherwise rank-0 only
    if Stages.TRAIN_SUPERNET in stages:
        used_dist = _stage2_init_dist_if_needed(global_settings)  # [ADDED] init only for Stage-2

        # everyone can construct from the same JSON safely
    ...
    # -- STAGE 3: Search Supernet (evolutionary search) (rank-0 only)  [MODIFIED]
    if Stages.EVO_SEARCH in stages and is_rank0_env():  # [MODIFIED]
    ...
    # -- STAGE 4: Fine tune the best solution (rank-0 only)  [MODIFIED]
    if Stages.FINE_TUNE in stages and is_rank0_env():  # [MODIFIED]
