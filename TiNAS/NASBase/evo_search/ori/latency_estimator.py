           ...
           # print(net_obj)
            if USE_TS:
                if self.global_settings.NAS_SETTINGS_GENERAL['MODE'] == 'pdq':
                    under_mem, peak_for_all, total_pdq_config, mac_count, access_gap  = op_duplication_subnet(self.global_settings, net_input, net_obj)

                elif self.global_settings.NAS_SETTINGS_GENERAL['MODE'] == 'tinyts':
                    under_mem, peak_after_sp, micrographs, mac_count, access_gap = op_duplication_subnet(self.global_settings, net_input, net_obj)
                

                elif self.global_settings.NAS_SETTINGS_GENERAL['MODE'] == 'tinynas':  ## wait for new version
                    under_mem, peak_after_patch, min_lat_mem_result, mac_count, access_gap = op_duplication_subnet(self.global_settings, net_input, net_obj)
