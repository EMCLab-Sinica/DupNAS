...
class EvolutionFinder:
    ...
    def check_constraints(self, sample):
        ...
        if not self.global_settings.NAS_EVOSEARCH_SETTINGS['EVOSEARCH_BYPASS_EFFICIENCY']:
            ...
            # if cached_nvm_fit != None and under_mem != None: # use cached value
            #     all_layers_fit_nvm = cached_nvm_fit
            #     if not cached_nvm_fit:
            #         print("cached_nvm does not fit! ") #skip subnet, over constraint
            #         return None, None
            #     if not under_mem :
            #         print("cached_vm does not fit! ") #skip subnet, over constraint
            #         return None, None
                ...
                if USE_TS:      
                    if self.global_settings.NAS_SETTINGS_GENERAL['MODE'] == 'pdq':
                        ...
                        if not under_mem or not cached_nvm_fit:
                            print("[pdq] TS doesn't help! ") #skip subnet, over constraint
                        ...
                        if not under_mem or not cached_nvm_fit:
                            print("[tts] TS doesn't help! ") #skip subnet, over constraint
                            self.evo_memory.update_tbl(sample, EvoMemTypes.NVM_FIT, cached_nvm_fit)
                            self.evo_memory.update_tbl(sample, EvoMemTypes.VM_FIT, under_mem)
                            return None, None #skip subnet, over constraint
                        else:
                        ...
                        if not under_mem or not cached_nvm_fit:
                            print("[tnas] TS doesn't help! ") #skip subnet, over constraint
                            self.evo_memory.update_tbl(sample, EvoMemTypes.NVM_FIT, cached_nvm_fit)
                            self.evo_memory.update_tbl(sample, EvoMemTypes.VM_FIT, under_mem)
                            return None, None #skip subnet, over constraint
                        else:
        ...
        with torch.no_grad():
            ...
            if self.global_settings.NAS_SETTINGS_GENERAL['MODE'] == 'pdq':
                ...
                if which_over_mem == [] and oripeak <= (vm_available*1024):
                    print("small enough, do not need tinyts")
                    under_mem = True
